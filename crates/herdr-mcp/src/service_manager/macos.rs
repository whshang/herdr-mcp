...
fn run_with_optional_mutation_lock(
    ...
    let paths = ServicePaths::discover()?;
    let owned_mutation_lock =
        if service_command_requires_mutation_lock(&command) && external_mutation_lock.is_none() {
    ...
    let scheduler = install_post_commit(
        ...
        || {
            integration_prior.restore(|| {
                crate::product_lifecycle::restore_installation_identity(identity_prior.as_deref())
            })
        },
    )?;
    if let Some(object) = result.as_object_mut() {
...
fn guardian_state_lock_path(paths: &ServicePaths, transaction_id: &str) -> Result<PathBuf, String> {
...
fn read_guardian_record(
    ...
    let bytes = read_optional_bounded(&guardian_record_path(paths, transaction_id)?, 64 * 1024)?
...
fn guardian_decision(
    ...
    match record.mode {
        ...
        GuardianMode::Rollback => {
            ...
            if rollback.rollback_id != rollback_id {
                return GuardianDecision::Refuse("guardian rollback identity mismatch".to_owned());
            }
            let candidate_active = active.map(|generation| generation.generation_id.as_str())
                == record.candidate_generation_id.as_deref();
            match rollback.state.as_str() {
                ...
                "consuming" | "rollback_failed" if candidate_active => GuardianDecision::Recover,
...
fn guardian_restore_known_good(
    ...
    if record.watchdog_was_loaded && watchdog_bytes.is_none() {
        return Err("guardian expected a loaded watchdog but has no watchdog backup".to_owned());
    }
...
fn guardian_after_parent_exit(paths: &ServicePaths, transaction_id: &str) -> Result<Value, String> {
...
fn guardian(paths: &ServicePaths, transaction_id: &str, parent_pid: u32) -> Result<Value, String> {
    if !guardian_fd_is_open(GUARDIAN_PARENT_FD) || !guardian_fd_is_open(GUARDIAN_LOCK_FD) {
        return Err(
            "guardian requires inherited parent-signal and mutation-lock descriptors".to_owned(),
        );
    }
    ...
    if record.state == "aborted" {
        return Ok(json!({"ok": true, "guardian": "aborted", "transaction_id": transaction_id}));
    }
    ...
    let parent_closed = wait_for_guardian_parent_signal(GUARDIAN_PARENT_FD, GUARDIAN_MAX_LIFETIME)?;
    ...
    Healthy: FnOnce() -> bool,
{
    ...
    let mut guardian = match arm_guardian(
        ...
        Err(error) => {
            if let Some(rollback_id) = rollback_id.as_deref() {
                let _ = store.mark_prepared_service_rollback(rollback_id, "aborted", now_ms_i64());
            }
            return Err(format!(
    ...
    let user_cli = user_cli_link
        .ok_or_else(|| "user CLI link missing after successful service activation".to_owned())?;
...
pub(super) fn rollback_target_runtime_binary() -> Result<Option<PathBuf>, String> {
    ...
    let target = rollback
        ...
        .as_deref()
        .map(PathBuf::from)
        .ok_or_else(|| "Rust rollback source has no previous runtime/current target".to_owned())?;
    if !is_owned_generation_target(&target) {
...
fn rollback(paths: &ServicePaths, mutation_lock: &ServiceMutationLock) -> Result<Value, String> {
    ...
    if !matches!(source.kind, ServiceKind::Node | ServiceKind::Rust) {
        return reject_ready("rollback server backup is not an owned Node/Rust service".to_owned());
    }
    ...
    if previous_target
        ...
        .is_some_and(|target| !is_owned_generation_target(target))
    {
        return reject_ready("rollback previous runtime/current target is not managed".to_owned());
    }
    ...
    if rollback != expected_claimed {
        let release = store.finish_service_rollback(&rollback.rollback_id, "ready", now_ms_i64());
        let settlement = guardian.finish(
            "parent_recovered",
    ...
    if let Err(error) = apply {
        ...
        return Err(match restore_error {
            ...
            None => {
                format!("service rollback failed but current Rust service was restored: {error}")
            }
        });
    }
    ...
    let guardian_settled = guardian
        ...
        .is_ok();
    let native_host_sync = if paths.instance.is_named() {
        false
    } else {
...
fn require_rust_service(paths: &ServicePaths) -> Result<ServiceDescriptor, String> {
    ...
    if paths.watchdog_plist.exists() || is_loaded(&paths.watchdog_label) {
        return Err("legacy watchdog is present; refusing dual supervisor ownership".to_owned());
    }
    Ok(descriptor)
}
...
fn prune_obsolete_generations(paths: &ServicePaths) -> Result<GenerationGcReport, String> {
    ...
    {
        let entry = entry.map_err(|error| format!("cannot inspect runtime generation: {error}"))?;
...
fn restore_current(paths: &ServicePaths, previous: Option<&Path>) -> Result<(), String> {
    match previous {
        Some(previous) => {
            ...
            symlink(previous, &temp)
                .map_err(|error| format!("cannot create rollback generation symlink: {error}"))?;
            fs::rename(&temp, &paths.current_link)
...
fn health_payload_matches_legacy(value: &Value, expected_version: &str, expected_pid: u64) -> bool {
    value.get("ok").and_then(Value::as_bool) == Some(true)
...
fn mcp_discover_once(descriptor: &ServiceDescriptor, port: u16) -> bool {
    ...
    client
        ...
        .body(r#"{"jsonrpc":"2.0","id":"service-health","method":"server/discover","params":{}}"#)
        .send()
        .ok()
        .filter(|response| response.status().is_success())
        .and_then(|response| response.text().ok())
...
fn wait_for_generation_health(
    ...
    let deadline = Instant::now() + HEALTH_BUDGET;
    while Instant::now() < deadline {
        if health_once_for_identity(port, Some(generation), legacy_health_version, service_label) {
            return Ok(());
        }
        thread::sleep(Duration::from_millis(150));
    }
...
mod tests {
    ...
    fn generation_gc_retains_current_and_ready_rollback_previous() {
        ...
        store
            ...
            .unwrap();
        store
            .activate_runtime_generation_with_rollback("rust-current", Some("rb-generation-gc"), 40)
            .unwrap();
        drop(store);
