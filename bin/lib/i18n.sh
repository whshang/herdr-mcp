...
T() {
  ...
  case "$HERDR_MCP_LANG:$key" in
    ...
    en:public_unset) echo "public MCP: HERDR_MCP_BASE_URL not set (deploy the Cloudflare Worker Edge first)" ;;
    en:public_ok) echo "public MCP: reachable" ;;
    en:public_down) echo "public MCP: unreachable (check Worker/Link/OAuth)" ;;
    ...
    en:no_base_url) echo "HERDR_MCP_BASE_URL not set — see README (Cloudflare Worker Edge)" ;;
    ...
    zh:public_unset) echo "公网 MCP: 未配置 HERDR_MCP_BASE_URL (先部署 Cloudflare Worker Edge)" ;;
    zh:public_ok) echo "公网 MCP: 可达" ;;
    zh:public_down) echo "公网 MCP: 不可达 (检查 Worker/Link/OAuth)" ;;
    ...
    zh:no_base_url) echo "未配置 HERDR_MCP_BASE_URL — 见 README（Cloudflare Worker Edge）" ;;
    ...
    ja:public_unset) echo "公開 MCP: HERDR_MCP_BASE_URL 未設定（Cloudflare Worker Edge を先にデプロイ）" ;;
    ja:public_ok) echo "公開 MCP: 到達可" ;;
    ja:public_down) echo "公開 MCP: 到達不可（Worker/Link/OAuth を確認）" ;;
    ...
    ja:no_base_url) echo "HERDR_MCP_BASE_URL 未設定 — README（Cloudflare Worker Edge）を参照" ;;
