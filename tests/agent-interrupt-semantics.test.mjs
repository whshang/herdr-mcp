...
test('working Agent stop semantics use terminal control, never a natural-language prompt', () => {
  ...
  assert.match(plannerSkill, /only if the Agent is still `working`, send `keys:\["CTRL_C"\]`/);
...
test('dynamic native method guidance exposes ESC then verify then CTRL_C', () => {
  assert.match(schema, /method === "agent\.send_keys"/);
  assert.match(schema, /keys=\[\\"ESC\\"\].+agent\.get or herdr_since.+keys=\[\\"CTRL_C\\"\]/s);
