...
test("fetchHerdrSkill offline mode returns bundled project policy plus live runtime context", async () => {
  ...
  try {
    ...
    assert.match(r.content, /\.claude\/skills\/\*\/SKILL\.md/);
    ...
    assert.match(r.content, /immediate.*read-only.*recovery check/s);
    assert.match(r.content, /3 seconds.*second read-only recovery check/s);
    assert.match(r.content, /7 seconds.*third and final read-only recovery check/s);
    assert.match(r.content, /roughly.*15–20 seconds total/s);
    assert.match(r.content, /bounded three-check recovery window/s);
    ...
    assert.doesNotMatch(r.content, /5 seconds.*10 seconds.*20 seconds|35 seconds|~35s/s);
