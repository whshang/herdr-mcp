...
test("ChatGPT artifact core parses the real plural conversation image shape and bounds a turn", () => {
  assert.ok(core);
  const payload = {
    messages: [
      { id: "u1", author: { role: "user" }, content: { parts: ["draw"] } },
      {
        id: "a1",
...
test("extension source keeps Bearer private while both authenticated download stages use it", async () => {
  ...
  assert.equal(manifest.version, "0.1.81");
