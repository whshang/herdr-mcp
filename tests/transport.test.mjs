...
//  6. HERDR_MCP_ALL_TOOLS=1 restores the full 31-tool surface (advanced +
//     deprecated), while the default surface stays exactly 18.
//
...
test("stateless: initialize then calls WITHOUT session id (ChatGPT pattern)", async () => {
  ...
  try {
    ...
    assert.equal(list.msg.result.tools.length, 18, `default tools/list must expose exactly 18 tools, got ${list.msg.result.tools.length}: ${list.msg.result.tools.map((t) => t.name).join(",")}`);
    for (let i = 0; i < 3; i++) {
...
test("keep-alive: initialize -> tools/list -> tools/call strictly framed on ONE raw TCP connection", async () => {
  ...
  try {
    ...
    // 2) tools/list with the session id — exactly the default 18-tool surface
    ...
    assert.equal(listMsg.result.tools.length, 18, `default tools/list must be exactly 18, got ${listMsg.result.tools.length}`);
...
test("openai-mcp UA: sessionless GET probe on / and /mcp -> 200 persistent SSE, no session, no EOF", async () => {
  ...
  try {
    ...
    assert.equal(list.status, 200);
    assert.equal(list.msg.result.tools.length, 18);
  } finally {
    sessionId = savedSid;
  }
});
...
test("HERDR_MCP_ALL_TOOLS=1 restores the full 31-tool surface (advanced + deprecated)", async () => {
  ...
  try {
    ...
    // the default 18 remain registered too
    const defaults = ["herdr_inspect", "herdr_skill", "herdr_since", "herdr_methods", "herdr_call", "herdr_prompt",
...
test("openai-mcp UA: initialize returns NO Mcp-Session-Id on / and /mcp (stateless end-to-end)", async () => {
  ...
  try {
    for (const p of ["/", "/mcp"]) {
      ...
      assert.equal(init.msg.result?.serverInfo?.version, SERVER_VERSION,
        `initialize serverInfo.version on ${p} must be ${SERVER_VERSION}`);
...
test("openai-mcp UA: initialize -> tools/list -> tools/call consecutive, stale sid tolerated", async () => {
  ...
  try {
    ...
    assert.equal(list.status, 200);
    assert.equal(list.msg.result.tools.length, 18,
      `openai-mcp tools/list must expose exactly 18 tools, got ${list.msg.result.tools.length}`);
...
test("openai-mcp UA: server restart on the same port -> stateless tools/call still succeeds (no Session terminated)", async () => {
  ...
  try {
    ...
    assert.equal(list.status, 200);
    assert.equal(list.msg.result.tools.length, 18, `tools/list after restart must stay 18`);
...
test(`server/discover (non-openai) result _meta serverInfo.version is ${SERVER_VERSION}`, async () => {
  for (const p of ["/", "/mcp"]) {
    const r = await fetch(`${BASE}${p}`, {
      method: "POST",
    ...
    assert.equal(si.version, SERVER_VERSION,
      `discover serverInfo.version on ${p} must be ${SERVER_VERSION}, got ${si.version}`);
  }
});

// Cache-key regression: an OpenAI client that previously cached a 0.2.0
// catalog MUST, upon seeing the current runtime identity (initialize serverInfo +
// mcp.json + discover), re-run tools/list and obtain the current 18 tools.
// We assert all identity surfaces agree on SERVER_VERSION and that a fresh tools/list
// returns exactly the 18 default tools (i.e. a re-fetch after a version bump
// does NOT resurrect the old 22-tool surface).
test(`cache-key regression: ${SERVER_VERSION} identity consistent, fresh tools/list = 18 (no stale 22)`, async () => {
  const savedSid = sessionId;
  sessionId = null;
  try {
    ...
    for (const p of ["/", "/mcp"]) {
      const init = await rpc("initialize", openaiInit, { noSession: true, ua: OPENAI_UA, path: p });
      assert.equal(init.msg.result?.serverInfo?.version, SERVER_VERSION, `initialize ${p} version`);
    ...
    assert.equal(cardJson.version, SERVER_VERSION,
      `mcp.json version must be ${SERVER_VERSION}, got ${cardJson.version}`);
  } finally {
    sessionId = savedSid;
  }
});
