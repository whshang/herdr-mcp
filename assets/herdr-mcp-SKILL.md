...
- treat obvious repeated-output loops as a fault, not progress. If a working Agent repeats the same or near-identical block several times (especially self-reports such as context corruption) with no new command result, Git diff, or task evidence, read a bounded `recent_unwrapped` pane/agent tail to confirm it, then interrupt the **current execution** instead of waiting for self-recovery. `agent.prompt` / `herdr_prompt` is business input and has **no stop/cancel semantics**; never send a new natural-language prompt such as “stop”, “cancel”, “停止”, or “不要继续” to interrupt a working Agent. Use the existing Herdr terminal-control sequence: (1) `herdr_call(method="agent.send_keys", params={"target":"<agent-or-pane>","keys":["ESC"]})`; (2) immediately re-read fresh state with `agent.get` and/or `herdr_since`; (3) only if the Agent is still `working`, send `keys:["CTRL_C"]`; (4) re-read fresh state again. Treat only `idle`, `blocked`, or `done` as settled enough for resource reclamation; `working` or `unknown` is not stopped. An interrupt proves only that control input was sent and the later status observed—it never proves a mutation had no side effects. If mutation delivery/outcome is uncertain, inspect Git/task/runtime state and do not replay the original mutation. `pane.close`/`tab.close` are cleanup operations after verification, never substitutes for interrupt and never cancellation proof. Before retrying work, inspect Git/task state so an already-applied mutation is not duplicated. If work remains, restart the task in a fresh Agent session/pane from the current verified Git state and the original task objective; do not continue the corrupted session. If the work was already captured or integrated, stop the Agent and do not rerun it. A second loop on the restarted task is a reassign/main-planner takeover signal, not permission for another blind retry;
