...
3. **Artifact/file ingress — always take the shortest safe path** (v0.4.2 built-in policy):
   ...
   Device enrollment is a **Worker/fleet administration** operation; enrolled devices have no owner/member hierarchy. If the current WebChat was explicitly approved by this Worker, create a short-lived pairing at Edge with `herdr_call(method="herdr_mcp.device.pair", params='{"ttl_seconds":600,"name":"<optional>"}')`. `herdr_call.params` is a JSON string in the frozen public schema. This method is Edge-local (no device selector and no workstation routing), returns the pairing address, one-time 6-digit code, exact expiry, and copyable `herdr-mcp worker connect <pairing_address>` command. If it returns `fleet_admin_required`, create the pairing on **any already-enrolled computer** with `herdr-mcp worker pair`; never run `worker pair` on the fresh computer as a discovery probe. If no enrolled device or explicitly approved WebChat exists because this is the first Worker, complete the first-Worker Cloudflare bootstrap before pairing. Default pairing TTL to 600 seconds unless the user requests shorter, show the exact expiry, and never persist pairing data to Git, shell history, ordinary logs, or unattended automation.
   To permanently revoke an enrolled computer from an explicitly approved WebChat, first call `herdr_devices`, select the exact immutable `device_id`, then call `herdr_call(method="herdr_mcp.device.revoke", params='{"device_id":"dev_...","confirm":true}')`. This is also a Worker fleet-admin action and does not route through a workstation. Never revoke by display name or pane/workspace ref. Revoke is permanent for that device identity and credential; re-enrollment requires a new pairing and device identity. If the WebChat lacks fleet-admin authority, run `herdr-mcp worker revoke <device-id> --confirm` from any enrolled computer.
...
- Keep one canonical reusable `herdr-mcp:utility` pane when the workspace uses `herdr_exec`; do not churn that pane as ordinary completion cleanup. Extra stale utility panes may be reclaimed only after proving they are not the canonical busy/owned utility surface.
...
- Unattended external callers such as GitLab CI must use a separately provisioned **Automation Client**, never a shared fleet-wide bearer and never the local `HERDR_MCP_TOKEN`. Create one trust boundary per project/environment with `herdr-mcp automation create --name <name>` on any enrolled workstation. The command returns a stable `client_id` plus a `client_secret` exactly once; store them as masked/protected CI variables (normally `HERDR_MCP_CLIENT_ID` and `HERDR_MCP_CLIENT_SECRET`) together with `HERDR_MCP_URL`. Each job exchanges those credentials at `/oauth/token` with `grant_type=client_credentials` for a short-lived access token (maximum one hour, no refresh token), then uses that access token on `/mcp`.
- Automation Clients are ordinary MCP principals, not fleet administrators. They cannot create/revoke devices, approve/revoke Connectors, or mint another Automation Client. `herdr-mcp automation list` returns non-secret inventory/issuance metadata; `automation rotate ... --confirm` replaces the long-lived secret immediately; `automation revoke ... --confirm` fences both future minting and already-issued access tokens. Through an approved WebChat, use the private `herdr_mcp.automation.list` / `herdr_mcp.automation.revoke` methods for inspection/revocation only; do not return a long-lived automation secret into the chat transcript by default.
