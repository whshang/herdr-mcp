...
2. **Direct workstation operations first**:
   ...
   - one short shell/build/probe: `herdr_exec`;
   - 2–8 ordered short shell steps, all known up front in the same workspace/project and requiring no model decision between steps: one `herdr_call(method="herdr_mcp.exec.sequence", ...)`; prefer typed `herdr_fs_*` / `herdr_git` instead when they express the operation directly;
...
- Do not spend one model round trip per short shell step when a bounded sequence is already known. When the live runtime advertises `private_exec_sequence`, use `herdr_mcp.exec.sequence` for 2–8 ordered steps in one workspace/project. It is intentionally sequential and fail-fast by default; it does not authorize cross-project work, parallel mutation, auto-retry, or a second scheduler. Per-step state and `[herdr-step:<id>]` markers preserve execution observability, including after a synchronous timeout when the returned `session_id` is continued with `herdr_exec_read`.
