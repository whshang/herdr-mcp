...
Worker order is therefore: deterministic `fs/git/exec` first → Herdr-native cheap worker (Pi preferred) → DSH headless as a long-running CLI fallback → interactive `dsh-tui` only when a human operator wants to take over. Recheck DSH after upgrades because it is still a fast-moving developer preview.
