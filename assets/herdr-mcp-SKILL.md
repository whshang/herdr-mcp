...
3. **Artifact/file ingress — always take the shortest safe path** (v0.4.2 built-in policy):
   ...
   To pair a new computer directly at Edge without requiring an enrolled or online workstation, call `herdr_call(method="herdr_mcp.device.pair", params='{"ttl_seconds":600,"name":"<optional>"}')`. `herdr_call.params` is a JSON string in the frozen public schema. This Edge-local method requires no device selector, creates a short-lived pairing session, and returns the pairing address, one-time 6-digit code, exact expiry, and copyable `herdr-mcp worker connect <pairing_address>` command. When the user asks conversationally to add/configure/pair a new computer or generate its setup link, prefer this Edge-local flow; default to 600 seconds unless they request a shorter TTL, clearly show the expiry, and never persist the short-lived pairing data to Git, shell history, ordinary logs, or unattended automation.
   To permanently revoke an old enrolled computer directly from an OAuth-authorized owner conversation, first call `herdr_devices`, select the exact immutable `device_id`, then call `herdr_call(method="herdr_mcp.device.revoke", params='{"device_id":"dev_...","confirm":true}')`. This method is also Edge-local and does not require any workstation to be online. Never revoke by display name or pane/workspace ref. Revoke is permanent for that device identity and credential; re-enrollment requires a new pairing and device identity. The owner CLI `herdr-mcp worker revoke <device-id> --confirm` remains the fallback.
...
The browser extension is the reverse/wake channel and stays on the local machine. It is **never binary/file transport**; the artifact/file ingress path is the local tools + direct signed import + private R2 relay ladder in section 1. Current installs send bounded request/stream messages to the registered Chrome Native Messaging host, which reaches herdr-mcp through `~/.config/herdr-mcp/extension.sock` (mode `0600`). The browser receives and stores no Herdr bearer. Static `HERDR_MCP_TOKEN` remains for other local clients and for the native host's old-runtime HTTP fallback only. The extension does not need the public Worker/OAuth URL. Do not route extension traffic through Cloudflare merely because the Connector uses Cloudflare.
