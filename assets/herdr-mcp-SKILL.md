...
4. **Use a development agent only when reasoning or parallel investigation is genuinely useful**. Prefer cheap/fast Herdr-native workers (`pi`, `cline`, `opencode`, `anti`) for implementation/investigation. Use `droid`/`grok` as independent auditors, not as the primary author by default.
...
6. **The web model remains the planner**. Do not ask a local Claude/OMP/main agent, Pi, or DSH to become a middle manager or to dispatch other agents for you.
