...
- prefer fire-and-forget, then observe with `herdr_inspect`/`herdr_since`; `herdr_prompt` returns a stable `dispatch_id` for an idempotent intent plus `dispatch_correlation`; treat `awaiting_activity` as accepted/submitted without observed new work, and treat `unverified_preexisting_work` / `unverified_preexisting_blocked` as non-terminal because native Herdr lifecycle state does not identify an exact prompt turn;
