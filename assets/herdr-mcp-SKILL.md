...
Within a wave, do not re-enter model planning between deterministic steps whose arguments and safety are already known. Put same-boundary shell/Git steps plus their local checks into one bounded `herdr_exec` when that is safer and cheaper than N remote calls **and the resulting tool argument remains narrow and single-purpose**. For read-only inspection, do not combine unrelated file, search, remote-host, or datastore probes merely to reduce request count. Re-plan only when new evidence changes later arguments/safety, a human action is required, or mutation delivery becomes uncertain.

WebChat/Connector hosts may reject a tool invocation before it reaches Herdr. When the client returns only a host-side safety rejection and none of Herdr's normal execution identity/evidence fields (for example `op_id`, `session_id`, `backend`, `pane_id`, `exit_code`, or `device_id`), classify the attempt as **pre-delivery**: do not attribute it to the Herdr runtime, SSH transport, filesystem, database, or remote service. One identical retry is allowed because the host rejection is explicit pre-delivery evidence. For a mutation, retry only when the host rejection itself proves pre-delivery and preserve the same idempotency key when the tool supports one; after a second host rejection, stop instead of changing transport, encoding, command shape, or tool power to force the mutation through. Do not build a tool/command blacklist from a single rejection. For read-only inspection that remains blocked, preserve the requested intent while reducing argument surface: prefer `herdr_fs_read` / `herdr_fs_list` / `herdr_fs_grep` and `herdr_git` for managed-project data; where remote maintenance inspection is actually necessary, use one bounded single-purpose read/probe per call instead of mixing several unrelated reads/searches in one command. If a structured read is itself rejected, narrow the exact file/range/pattern when that still answers the same question. This is planner-side request shaping, **not permission to bypass, disable, or evade host safety checks**; never escalate to a more powerful tool solely to force a rejected request through.

When a direct Herdr operation remains host-blocked after the bounded retry, a compatible **existing local Agent** may be used as an execution fallback when the task is genuinely suitable for autonomous local work. Use `herdr_prompt` with one high-level task contract (desired outcome, allowed scope, relevant project/context, required verification, and no further delegation) instead of copying, encoding, obfuscating, or mechanically rephrasing the rejected shell/API payload. For a mutation, take this fallback only when the original host rejection proves pre-delivery; keep the user's original authorization/confirmation boundary unchanged and verify the target system state afterward with the safest available read path. Agent fallback is an alternate execution subject, **not a privilege or safety bypass**: it must not expand the requested scope, obtain capabilities the user did not authorize, or convert an irreversible action into an implicit approval. If `herdr_prompt` is itself rejected by the host before Herdr delivery, stop the fallback chain and report that both direct execution and Agent delegation were blocked before reaching Herdr; do not keep wrapping the same intent through stronger tools or alternate encodings.
...
- Network-bound file transfers whose completion time depends on remote latency or artifact size count as long work by default, even when the command itself is deterministic. This includes GitHub Actions artifact downloads via `gh api` / `gh run download`, `curl`/`wget`, `scp`/`rsync`, and comparable uploads/downloads. Start them once with `herdr_exec_start`, then poll that same session with `herdr_exec_read`; do not spend the synchronous `herdr_exec` timeout budget hoping the transfer finishes within 60 seconds. If a transfer was mistakenly started synchronously and returns timeout or uncertain delivery, first inspect the original process/session and destination-file growth; never start a duplicate transfer until evidence proves the original is no longer running and replacement is safe.
...
- Herdr 0.9 machine selection is currently a TUI concern, not a machine-scoped socket/CLI selector. Do not assume a selected saved machine changes where ordinary `herdr pane ...` / `herdr workspace ...` commands execute, and do not compose `herdr --remote <target>` with pane/workspace subcommands unless the live Herdr schema/help explicitly supports it. For current programmatic maintenance/UAT, read `herdr machine list --json`, resolve the intended profile's SSH target + Herdr session, execute Herdr CLI on that server explicitly, then re-read remote workspace/pane ids before acting.
...
- When upstream Herdr adds native machine-scoped pane/workspace addressing, detect it from live help/schema/capabilities and prefer that path over the SSH CLI bridge; do not remove Edge device affinity or collapse the two identity namespaces.
...
- Prefer one canonical reusable `herdr-mcp:utility` pane when the workspace uses `herdr_exec`; avoid pane proliferation when the current utility surface is available. For the same WebChat conversation/task and the same workspace/project, keep routing bounded shell work through that pane and batch already-known same-boundary short commands into one `herdr_exec` wave. Do not use `pane.split` or `herdr_exec_start` merely to get another shell: `herdr_exec_start` is a durable session and should be started once only for genuinely long work. On macOS privacy-protected roots it reuses that workspace's canonical utility pane instead of splitting a fresh pane per command; ordinary roots keep the native background-process path. Bounded SSH maintenance/recovery follows the same rule: keep it in the controlling workspace's canonical utility pane; long remote work uses one durable session and the same `session_id`. This is an advisory efficiency rule, not an absolute single-pane lock: when an unrelated task already owns or blocks the current utility/exec surface and the current task is urgent or genuinely independent, opening another bounded utility/exec lane is allowed. Keep those extra lanes scoped to the conflicting activity, then reclaim planner-created extras after the work is captured and verified. Do not churn the canonical reusable pane as ordinary completion cleanup, and reclaim any stale utility pane only after proving it is not the canonical busy/owned surface.
