...
8. Load domain Skills only when required and keep them sticky while source identity/digest are unchanged. Do not load `herdr_skill` as a session heartbeat; omit its native Herdr reference unless native CLI semantics are needed.
