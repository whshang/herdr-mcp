...
- Supported private methods with **no CLI wrapper**: `browser_dispatch.stop`, `browser_endpoint.inspect`, `browser_space.inspect`. `browser_session.open` is available through `herdr-mcp webchat open`; `browser_handoff.prepare` is reached through `herdr-mcp webchat handoff`. `webchat create` still cannot pass `source_url`.
