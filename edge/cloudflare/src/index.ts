 ...
 *   GET  /semantic/status  POST /semantic/systemone|chat  optional enrolled-device semantic proxy
...
import { handleSemanticProxy } from "./semantic-proxy.js";
import {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ---- Optional semantic fast path. The Worker keeps the provider secret;
    // enrolled workstations only receive the bounded semantic result.
    if (
      (request.method === "GET" && url.pathname === "/semantic/status")
      || (request.method === "POST" && url.pathname === "/semantic/systemone")
      || (request.method === "POST" && url.pathname === "/semantic/chat")
    ) {
      return handleSemanticProxy(
        request,
        env,
        url.pathname === "/semantic/chat" ? "chat" : "evaluate",
      );
    }
    ...
    if (request.method === "GET" && url.pathname === "/info") {
      return jsonResponse({
        ...
        routes: [
          ...
          { path: "/semantic/status", stage: "optional semantic provider capability probe" },
          { path: "/semantic/systemone", stage: "optional enrolled-device semantic proxy" },
          { path: "/semantic/chat", stage: "optional enrolled-device semantic chat proxy" },
    ...
    if (request.method === "POST" && url.pathname === "/connectors/webchat-control") {
      ...
      const allowedKeys = new Set(["connector_id", "device_id", "endpoint_ref", "provider", "account_ref", "allowed"]);
      ...
      if (
        !/^conn_[A-Za-z0-9_-]{8,128}$/.test(connectorId)
    ...
    if (request.method === "POST" && url.pathname === "/connectors/page-assist") {
      ...
      const allowedKeys = new Set(["connector_id", "device_id", "endpoint_ref", "allowed"]);
      if (Object.keys(parsed.value).some((key) => !allowedKeys.has(key))) {
        return noStoreJsonResponse({ ok: false, code: "bad_request" }, 400);
      }
      const connectorId = typeof parsed.value.connector_id === "string" ? parsed.value.connector_id.trim() : "";
...
async function verifyEdgeAccessToken(env: Env, token: string): Promise<{
  ok: boolean;
  clientId?: string;
  connectorId?: string;
  ...
  const connectorId = typeof payload.connector_id === "string" && /^conn_[A-Za-z0-9_-]{8,128}$/.test(payload.connector_id)
    ? payload.connector_id
    : undefined;
  const grantGeneration = Number.isSafeInteger(payload.grant_generation) && Number(payload.grant_generation) > 0
