...
import {
  ...
  normalizeOAuthApproval,
  normalizeOAuthClient,
  normalizeOAuthCode,
  normalizeOAuthToken,
  normalizePageAssistGrants,
  normalizeWebChatControlGrants,
  record,
...
import type {
  ...
  OAuthPageAssistGrant,
  OAuthTokenRecord,
  OAuthWebChatControlGrant,
} from "./oauth-records.js";

// Preserve the historical `./oauth-store-do.js` surface: the Durable Object
// module stays the import path for record types and for the exported
// normalizers that callers and tests already rely on. The record shapes and
// their normalization rules live in ./oauth-records.js.
export type {
  OAuthApprovalRecord,
  OAuthClientRecord,
  OAuthCodeRecord,
...
export { normalizeOAuthClient, normalizeOAuthToken, normalizeOAuthCode };
...
export class OAuthStoreDO {
  ...
  private async revokeGrant(request: Request): Promise<Response> {
    ...
    const current = raw ? normalizeConnectorGrant(raw) : null;
    if (raw && !current) return json({ ok: false, code: "invalid_record" }, 500);
    ...
    const hasConnector = [...connectors.values()].some((rawConnector) => {
    ...
    return json({
      ...
      legacy_tombstone_created: raw === undefined && client === undefined && !hasConnector,
    });
  }
