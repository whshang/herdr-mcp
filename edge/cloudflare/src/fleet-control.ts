...
function normalizeStoredWebChatBinding(value: unknown): WebChatLaneBinding | null {
  ...
  if (!isRecord(value) || value.schema_version !== 1) return null;
  if (!boundedString(value.endpoint_ref, 96) || !boundedString(value.provider, 32) || !boundedString(value.account_ref, 96)) return null;
  if (!(value.space_ref === null || boundedString(value.space_ref, 96))) return null;
  if (!boundedString(value.session_ref, 96) || !integer(value.observation_generation, 1) || !integer(value.bound_at_ms, 0)) return null;
...
function normalizeWebChatBindingInput(value: unknown, nowMs: number): WebChatLaneBinding | null | undefined {
  ...
  if (!boundedString(value.endpoint_ref, 96) || !boundedString(value.provider, 32) || !boundedString(value.account_ref, 96)) return undefined;
  const spaceRef = value.space_ref === undefined || value.space_ref === null ? null : boundedString(value.space_ref, 96) ? value.space_ref : undefined;
  if (spaceRef === undefined) return undefined;
  if (!boundedString(value.session_ref, 96) || !integer(value.observation_generation, 1)) return undefined;
  return {
    schema_version: 1,
  ...
  return storage.transaction(async (tx) => {
    ...
    if (method === "herdr_mcp.work_chain.create") {
      ...
      if (method === "herdr_mcp.work_chain.checkpoint.update") {
        ...
        if (targetDeviceId !== lane.device_id && lane.webchat_binding && params.webchat_binding === undefined) {
