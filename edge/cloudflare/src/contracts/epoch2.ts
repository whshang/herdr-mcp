/** Frozen ChatGPT-visible MCP contract epoch 2, captured from the 18-tool current surface before the 0.3.32 rollout. */
export const EPOCH2_CONTRACT = {
  "contract_epoch": 2,
  "contract_hash": "sha256:7da23ad2ec8e7703d6380062126ba797218bde9e7711138c6b3e0ca6592efbf8",
  "tool_count": 18,
  "tools": [
    {
      "name": "herdr_methods",
      "description": "Before any agent operation (herdr_prompt or agent.* via herdr_call): call herdr_skill once per session first. Discover herdr socket API methods and parameter schemas. LIVE reflection from the installed herdr binary (herdr api schema, 60s cached). Use this when you don't know the exact method or argument names (especially before agent.* calls).",
      "inputSchema": {
    ...
    {
      "name": "herdr_inspect",
      "description": "Check herdr connection and list workspaces (with cwd), tabs, panes, and agents in one call. Typical session start: herdr_inspect → herdr_skill (read before any agent op) → work. Also returns workstation_info: default_cwd hints, server/build, readonly/write_roots, agent_skill pointer (call herdr_skill for latest upstream SKILL.md), and a short exec_environment summary (PATH binaries relevant to local coding). Agents come from the shared SnapshotCache (live events + 30s snapshot fallback) and carry started_at + last_activity_at. If session.snapshot blips (TaskGroup), falls back to workspace.list / pane.list / agent.list and sets warnings[] — do not treat that as a repo blocker; keep using herdr_fs_* / herdr_git / herdr_exec. YOU (web) are the planner/orchestrator. Prefer herdr_fs_* / herdr_exec / herdr_git before any herdr_prompt. Agent lists soft-hide expensive kinds (Claude/OMP/Codex); only allowlisted workers (pi, cline, opencode, anti) and auditors (droid, grok) appear — override with HERDR_MCP_AGENT_ALLOW. herdr_prompt by known name/pane_id is NOT blocked. Prefer explicit pane_id/workspace_id from this view.",
      "inputSchema": {
        "type": "object",
        "properties": {}
      },
    ...
    {
      "name": "herdr_skill",
      "description": "Read-only dynamic operating skill for a remote/web herdr-mcp planner. It combines: (1) the herdr-mcp project policy for tool order, direct fs/git/exec edits, cheap-agent dispatch, mutation/idempotency rules and self-upgrade; (2) live runtime / generation / self-update status; and, by default, (3) the installed release-matched `herdr --skill` as explicitly scoped native reference. The project policy has precedence because a web planner is outside HERDR_ENV. Project policy can refresh from the herdr-mcp repository with a bundled release fallback. Call once near session start and refresh after a herdr-mcp upgrade or when operating rules may have changed.",
      "inputSchema": {
        "$schema": "http://json-schema.org/draft-07/schema#",
        "type": "object",
        "properties": {
          "refresh": {
            "default": false,
            "description": "Bypass the project-skill cache and re-check the configured upstream policy",
            "type": "boolean"
          },
          "include_native_reference": {
            "default": true,
            "description": "Append release-matched `herdr --skill` output as scoped native reference; disable to save context",
            "type": "boolean"
          }
        }
      },
    ...
    {
      "name": "herdr_call",
      "description": "Before any agent operation (herdr_prompt or agent.* via herdr_call): call herdr_skill once per session first. Generic passthrough to the herdr socket API, VALIDATED against the live schema (schema reflected from the installed herdr binary, 60s cache). Params are checked before sending: missing required / wrong type / wrong enum -> invalid_params error (no socket call); unknown params -> warnings. For unknown methods use herdr_methods; for agent.* methods (agent.start, agent.prompt, pane.split, …) herdr_skill is mandatory once per session. prefer explicit pane_id/workspace_id over bare names. For agent.prompt prefer herdr_prompt (fire-and-forget + idempotency_key + delivery evidence); do not pass wait on mutations unless you intentionally want submit+wait. Never blind-retry a mutating call after failure — delivery may be uncertain; verify with herdr_inspect / herdr_since first. Status-wait timeouts are failure agent_status_wait_timeout (not herdr_transport).",
      "inputSchema": {
        "$schema": "http://json-schema.org/draft-07/schema#",
        "type": "object",
        "properties": {
          "method": {
            "type": "string",
            "description": "herdr socket method, e.g. pane.split, agent.start — read herdr_skill before agent.*"
          },
          "params": {
    ...
    },
    {
      ...
      "inputSchema": {
        ...
        "properties": {
          "target": {
            "type": "string",
            "description": "Agent name or pane_id to prompt (call herdr_skill first if new session)"
          },
          "text": {
            "type": "string",
