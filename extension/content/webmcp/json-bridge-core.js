...
(function (root) {
  ...
  function buildSystemPrompt(tools, siteName) {
    const siteHint = siteName === "deepseek"
      ...
      : "";
    return `${MARKER}\nYou have local Herdr tools through a browser JSON bridge. Use them whenever the user asks you to inspect, modify, run, verify, or continue work on the bound workstation.\n\nProject bootstrap:\n- As soon as a target project/repository root is known, before substantive project work (including read-only analysis), inspect that root for AGENTS.md, CLAUDE.md, and README.md.\n- Read every one of those files that exists. Missing files are normal. Follow project-local instructions within their scope and never reuse another project's instructions.\n- Do this bootstrap once per target project root before continuing with the task.\n\nTool-call protocol:\n- Emit one JSON object per line, exactly {\"tool\":\"<exact tool name>\",\"args\":{...}}.\n- Plan one high-yield wave at a time. Emit every independent call you already know you need in the same reply; the bridge executes up to 24 calls as one browser batch and returns one ordered TOOL_RESULT batch.\n- Prefer one broad-but-bounded inspect/grep/git/exec call over several narrower calls when it safely answers the same question. Do not repeat discovery already returned in the current task unless it may be stale.\n- Keep dependent steps sequential.\n- A tool-call reply contains JSON only: no Markdown fences and no prose around the JSON.\n- Never invent tool names or arguments. Use the typed catalog below.\n- After a TOOL_RESULT message, either emit the next JSON tool call(s) or answer the user normally.\n- Do not claim a tool succeeded until its TOOL_RESULT confirms success.\n- Prefer targeted fs/git/exec operations over expensive agent prompts when deterministic tools can do the work.${siteHint}\n\nTools (typed API):\n${schemaToTypedApi(tools)}`;
  }
