...
For ChatGPT Auto, deterministic browser/runtime safety gates stay authoritative. Ordinary post-turn semantic judgment uses a fixed progressive chain: **Jev/System One when configured → OpenAI-compatible LLM judge when configured → bounded script fallback**. Goal-aware automation additionally gives the existing LLM Goal Supervisor one bounded Jev semantic prior, while Work Memory/TODO evidence remains the completion authority. Users configure only each provider's endpoint, model, and API key; there is no user-selectable Shadow/Assist mode, probability threshold, judge prompt, or done-keyword list.
