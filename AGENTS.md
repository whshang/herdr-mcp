...
8. Semantic judgment is advisory policy, not a second execution authority. Browser Auto keeps its fixed Jev/System One → OpenAI-compatible LLM judge → bounded script fallback. The native runtime may expose bounded Noul/Choice/Score-style semantic advice through the provider-neutral `SemanticService` when a provider is configured; TypeSafe/Jev is the first provider and `TYPESAFE_API_KEY` only makes that capability available. Provider absence, timeout, HTTP failure, malformed output, or uncertainty must preserve the pre-semantic behavior. Planning may use semantic advice to conservatively fill otherwise-unspecified task requirements and rank Skill/method candidates; Work Memory may add a same-page relevance ranking without changing persisted hit/cursor order. Work Memory/TODO evidence and deterministic guards remain authoritative. Never let a semantic provider bypass uncertain-delivery, human-boundary, generation, permission, account isolation, idempotency, cleanup safety, or context-pressure gates. New fast-judgment models integrate as `SemanticProvider` adapters instead of adding provider-specific logic to business tools.
