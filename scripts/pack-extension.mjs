 ...
 * standalone: manual Load-unpacked package, with the public fixed standalone
 * manifest key injected into the staged copy without mutating extension/manifest.json.
 *
 * Both channels emit herdr-mcp-extension[-standalone]-<manifest.version>.zip
 * (+ .sha256 sidecar) with manifest.json at the zip root. These are maintainer
 * distribution artifacts; the normal product path installs STANDALONE through
...
/** Derive the Chromium extension ID from a base64 SubjectPublicKeyInfo `key`. */
export function extensionIdFromManifestKey(key) {
  if (!key || typeof key !== "string") {
    throw new Error("standalone manifest key is missing");
  }
...
export function injectManifestKey(sourceText, key) {
  ...
  if (closing < 0 || sourceText[closing] !== "}") {
    throw new Error("manifest has no closing object brace");
  }
  let last = closing - 1;
...
export async function packExtension({
  ...
  if (channel === "standalone") {
    ...
    const derived = extensionIdFromManifestKey(key);
    if (derived !== extensionId) {
      throw new Error(
        `standalone manifest key derives ${derived}, expected contract extension_id ${extensionId}`,
      );
    }
