...
1. 在整仓 gate 再次通过后，用 `service install --adopt-node` 做受控 production service-manager UAT：验证 8772 MCP、`extension.sock`、旧 Node Native Messaging compatibility host → Rust IPC、watchdog retirement、state v4 generation/rollback evidence；任何 browser/UAT 异常立即执行显式 `service rollback`；
2. service-manager production UAT 通过后再切真实浏览器 manifest/wrapper 到 Rust `extension-host`，验证 Native Messaging → Rust Unix IPC shared SSE、service-worker restart/reconnect、Project binding 恢复，并保留 `service rollback` 作为 runtime 层兜底；`agent_output` 等 enrichment 只有在现有浏览器行为确实依赖时迁移；
