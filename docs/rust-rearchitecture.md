...
1. 先对齐 PR #9 后的 release/recovery target contract：正式矩阵固定为 macOS ARM64 + Windows x64，`rust-release.yml`、manifest builder、`rust-release-recover.yml` 与相关 tests 必须从同一 authoritative target set 得出 expected assets，不能分别硬编码不同数量；通过 targeted tests 与 Grok audit 后，再从包含 PR #8/#9 和 alpha.4 version bump 的最新 `main` 发布 immutable `v0.4.0-alpha.4`：release gate → 两平台 build → manifest → `actions/attest` → GitHub Release；发布后对 manifest 与两个 binary 做独立 attestation verification，确认 tag、source commit、workflow、subject digest 与 Rekor evidence 一致；
2. 按 `AGENTS.md` 的 lifecycle 规则，在**独立 terminal/process** 做一次 alpha.4 trusted bootstrap，不能从 managed `herdr_exec` 内执行会 restart/activate `dev.herdr-mcp.server` 的 mutation。bootstrap 后重新确认 `runtime/current`、launchd exact label、health、version、epoch2/18 tools、state schema 和 rollback record；
3. alpha.4 稳定后再准备严格更高版本（预计 `0.4.0-alpha.5`，只在开始该 UAT 时 bump）做完整 updater production UAT：`check → apply queued → detached worker → generation activation → health → status`，并覆盖坏 checksum、坏 provenance/repo/workflow/ref/source revision、active-job 冲突、dead/interrupted worker recovery、重复 apply/idempotency、service health failure 自动 rollback、current/previous generation 正确性。普通 updater 继续强制 same `state_schema=4`；schema migration 另走 expand/contract handoff；
