...
1. 从最新 `main` 发布 immutable `v0.4.0-alpha.5`：先完成 version/docs sync、完整 release gate 与 Grok 审计，再做一次 non-tag `Rust Release` rehearsal，确认 authoritative target contract 只生成 macOS ARM64 + Windows x64 两个 build；随后 tag path 必须完整通过 `verify + targets（并行） → build → manifest → attest → publish`。发布后对 manifest 与两个 binary 独立验证 SHA-256、schema-v2 identity 与 GitHub/Sigstore attestation，不能只以 workflow success 代替本地验证；
2. 按 `AGENTS.md` lifecycle 规则，用已独立验真的 alpha.5 macOS ARM64 binary 在**独立 detached process**完成 `alpha.1 → alpha.5` trusted bootstrap。service install 必须先让 one-shot guardian 进入 watching；任何控制连接/worker 中断都只能恢复 known-good alpha.1，不能自动重试 candidate。事务结束后重新确认 `runtime/current`、launchd exact label、8772 health、version、epoch2/18 tools、state schema、active/previous generation、ready rollback、guardian transaction/event 和 mutation lock；
3. alpha.5 稳定后准备严格更高版本（预计 `0.4.0-alpha.6`）做完整 **P1 updater production UAT**：`check → download → provenance/SHA verify → stage → apply queued → detached worker → service install/guardian → generation activation → restart/health → status/commit`，并覆盖坏 checksum、坏 provenance/repo/workflow/ref/source revision、active-job 冲突、interrupted/dead worker recovery、重复 apply/idempotency、service health failure rollback、current/previous generation 正确性。普通 updater 继续强制 same `state_schema=4`；schema migration 另走 expand/contract handoff；
