...
>
> Relay/link Batch 6F 已完成 staged `RuntimeControlLoop`：新增 `crates/herdr-mcp/src/link/runtime_control.rs`，把 Node `src/link/runtime-control.ts` 的文件控制面移到 Rust。文档校验 schema/revision/唯一 generation id/`desired_active` 必须指向已登记代；`initialize` 在缺文件时写出 revision 1 基线文档；`tick` 按 revision 注册/激活并原子写回 status。完整 Rust **354/354 + guardian 1/1**，fmt、clippy `-D warnings` 与 diff-check 全绿。下一批进入 candidate-only Link 组装（`LinkIoLoop` + manager + control loop）；完成前生产 Link 继续走 Node。
