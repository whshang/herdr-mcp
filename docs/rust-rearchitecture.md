...
1. 完成 Rust supervisor / service manager，把 production/dev generation、HTTP listener、extension IPC ownership、health、restart evidence、稳定 launcher 与安装/卸载纳入同一生命周期；Native Messaging data plane 和 candidate install/status/uninstall 已完成，不再重复实现；
2. supervisor 稳定后再做真实浏览器上行 cutover：将 manifest/wrapper 从 Node compatibility host 切到 Rust `extension-host`，验证 Native Messaging → Rust Unix IPC shared SSE、service-worker restart/reconnect、Project binding 恢复与 rollback；`agent_output` 等 enrichment 只有在现有浏览器行为确实依赖时迁移；
