...
SSOT for gate rows: [`docs/ga-release-gate.md`](./ga-release-gate.md). Exit-alpha runbook (archived): [`docs/history/ga/exit-alpha-checklist.md`](./history/ga/exit-alpha-checklist.md). Release planes: [`docs/release-model.md`](./release-model.md). **Patch-line note:** current published Rust runtime stable remains `v0.4.1`; the `v0.4.2` source candidate includes the pre-publish legacy Dev Native Host updater migration discovered by candidate UAT. Its stable publication is still blocked on Developer ID release credentials **and the resulting signed launchd qualification of the macOS Documents/responsible-identity path**. The Rust Release manual-dispatch path is reserved for signed, attested pre-tag qualification and cannot publish; this file intentionally preserves the first-GA `v0.4.0` closure snapshot instead of rewriting historical evidence for each patch release.
