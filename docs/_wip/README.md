...
Current active WIP includes the v0.4.4 final closeout, the v0.4.5 follow-up maintenance plan, browser control plane settlement, DEV/STANDALONE/STORE extension distribution work, and modular progressive skills. The completed v0.4.3 release plan and multi-device core design are archived under `docs/history/architecture/`. After v0.4.4 is published, archive its release plan and leave v0.4.5 as the active release plan. Completed release/qualification work must not remain an active-plan label. Remove or archive a plan when its implementation decision is complete; do not leave completed evidence here for convenience.
