...
The semantic-judgment policy is on `main` at extension 0.1.103, and the provider-cleanup closeout merged extension 0.1.114 with supported Grok optional-site access, provider-scoped bounded recovery, endpoint-affine browser actuation, and dispatch-id-bound durable result settlement. The asynchronous Agent-task / durable parent-wake lane then advanced `main` to extension 0.1.115, and 0.1.116 unified Claude/Grok Project binding scopes. The current extension-changing lane advances the candidate to 0.1.117 so a user opening Herdr on a Grok tab can see the Project binding control before persistent Grok access exists, then grant the optional `grok.com` permission from the first Bind click. The same candidate treats Claude `/new` as a pending bindable page and migrates that binding only after Claude materializes a real conversation identity. Treat 0.1.117 as a candidate until this lane is merged and the exact STANDALONE package is qualified. Any later extension-changing PR has the same rule.
