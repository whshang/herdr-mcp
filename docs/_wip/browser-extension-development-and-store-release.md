...
The semantic-judgment policy is now on `main` at extension 0.1.103. The provider-cleanup closeout lane advances its exact candidate to 0.1.105 for supported Grok optional-site access, provider-neutral Grok result settlement, and final Claude/Grok signed-in qualification. Treat 0.1.105 as a candidate until that lane is merged. Any later extension-changing PR has the same rule.
