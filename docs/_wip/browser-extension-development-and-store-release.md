...
The semantic-judgment policy is on `main` at extension 0.1.103, and the provider-cleanup closeout merged extension 0.1.114 with supported Grok optional-site access, provider-scoped bounded recovery, endpoint-affine browser actuation, and dispatch-id-bound durable result settlement. The asynchronous Agent-task / durable parent-wake lane then advanced `main` to extension 0.1.115, 0.1.116 unified Claude/Grok Project binding scopes, and 0.1.117 added the direct Grok optional-permission flow. The current extension-changing lane advances the candidate to 0.1.118 with the same Grok behavior plus a compact Settings information architecture: common controls stay visible while experimental sites, Page Assist, connection details, continuity templates, and timing controls are collapsed until needed. Treat 0.1.118 as a candidate until this lane is merged and the exact STANDALONE package is qualified. Any later extension-changing PR has the same rule.
