...
- extension manifest version: **0.1.132**;
...
The current candidate keeps ChatGPT as the reference 1.0 provider while Claude and Grok retain signed-in dispatch, settled-result, and reload-recovery support. ChatGPT, Claude, and Grok are all required WebChat origins at install time; Grok no longer has a separate optional-permission toggle. Settings use visible cards instead of collapsed sections, put connection diagnostics first, keep Page Assist and the local Runtime URL out of user configuration, expose the local `~/.config/herdr-mcp/config.json` route format, and keep localized automation templates aligned with the selected UI language with an explicit reset action. Extension 0.1.122 restored the ChatGPT one-mention attachment contract by persisting the first provider-owned App pill observed on a bound turn as the binding's Herdr App identity. Extension 0.1.123 closes the late-binding edge after a binding change. Extension 0.1.124 makes bound Queue delivery reuse only a learned provider App identity and lets composer recovery match provider-owned `data-keyword` / `data-value` before visible menu text. Extension 0.1.125 removes the remaining fixed-name guess from existing-session handoff/recovery/Browser Actuation. Extension 0.1.126 preserves provider-owned App pills across Queue composer clearing, reasserts the learned App before resume-only submission, and carries one uniquely learned source App keyword through ChatGPT handoff/session.create into the committed target binding. Extension 0.1.127 removes the remaining fresh-session fixed-name guess, excludes provider-owned App pills from queued draft text, accepts pill-only composers on delayed Queue delivery, and lets the shared wake path reuse only explicit, learned, or currently selected provider-owned App identity. Extension 0.1.128 scopes App re-selection to the visible composer menu and adds one fail-closed ChatGPT keyword-search fallback when that menu does not initially list the App; the fallback runs only while the composer has no model-visible text. Extension 0.1.129 makes manual semantic/JEV Continue carry the authoritative binding App identity explicitly, closing the remaining content-script reload race. Extension 0.1.130 keeps the same App-identity invariant but hardens fresh ChatGPT Browser Actuation submission with one MAIN-world form requestSubmit and no replay after an uncertain result. Real #399 UAT then proved that ChatGPT Project home could consume that submit without producing provider acceptance. Extension 0.1.131 switched the bounded primitive to one MAIN-world send-button click, which also failed to produce provider acceptance in real UAT. Extension 0.1.132 therefore uses exactly one Enter key sequence on the already-proven composer, with no requestSubmit/click fallback inside the bounded helper and the same uncertain/no-replay rule. Unknown identity remains fail-closed; no generated turn invents an App name. Treat 0.1.132 as a candidate until this lane is merged and the exact STANDALONE package is qualified. Any later extension-changing PR has the same rule.

**ChatGPT App identity invariant.** Herdr never configures, guesses, or derives the ChatGPT App identity from a display label. The identity may be learned only after a ChatGPT conversation/Project is already Herdr-bound and the latest accepted user turn exposes exactly one provider-owned `ecosystemMention` pill with a concrete `data-keyword`; that keyword is persisted on the existing binding as `herdr_app_keyword` and becomes the durable authority for later Herdr-generated turns. A provider-owned App pill that is already selected in the live composer may be reused only as attachment evidence for that current turn; it does not create a second durable authority. Handoff, fresh `session.create`, Queue delivery, recovery, and binding migration may inherit only an explicit caller value or one unique persisted source-binding keyword. If identity is absent, non-unique, cannot be uniquely re-selected from the visible composer App menu, or cannot be read back after selection, Herdr fails closed and does not submit or replay the turn; there is no fixed `herdr` fallback and uncertain delivery is reconciled rather than blindly repeated.
