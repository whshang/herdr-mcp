...
For the v0.4.8 (schema 5) → v1.0 (current binary schema) boundary, the explicit migration path is `update major-apply`, run from a downloaded/verified v1.0 binary in an independent terminal. The release manifest derives the target schema from `state_store::SCHEMA_VERSION`; the exact target must pass real lifecycle qualification before publication. The v1.0 binary verifies that the live database is schema 5, stops the running source service so the final snapshot includes writes committed during shutdown, then creates a private consistent SQLite snapshot plus a private executable copy of the exact v0.4.8 source binary, hashes both rollback artifacts, and reuses the normal transactional service install to migrate and activate the target schema. A failed snapshot attempts to restart the source service and reports any restart failure, and preexisting rollback material is rejected before the service stop so a retry cannot overwrite an earlier backup. Keeping the rollback binary inside the private major-upgrade backup set makes rollback independent of later runtime-generation garbage collection. `update major-rollback` verifies the recorded artifacts, stops the active service, restores that exact pre-upgrade schema-5 snapshot, and reinstalls the recorded v0.4.8 binary. It also accepts a migration interrupted at any schema between 5 and 14, then retires the consumed backup material. This rollback intentionally discards durable state created after the v1.0 migration; use it only as an N-1 recovery operation. Both major lifecycle commands refuse managed `herdr_exec` execution because stopping/replacing the service from the control transaction that carries the command is unsafe.
