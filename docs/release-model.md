...
| --- | --- | --- |
| Runtime binary | `0.4.1` published / `0.4.2` source candidate | `v0.4.2` is not tagged. Stable TCC broker cross-generation authorization and the final production Artifact Relay/R2 UAT are verified. Remaining before tag: exact-final-source Rust Release qualification; Developer ID is optional hardening |
| Browser extension source | `0.1.81` | Current development source; Chrome Web Store published/review version may lag and must be checked independently |
