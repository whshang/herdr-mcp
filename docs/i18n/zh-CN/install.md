...
x86_64 Debian 服务器应使用 `herdr-mcp-<version>-x86_64-unknown-linux-musl` 产物，先通过 release manifest 校验 checksum / attestation，再把原始 binary 安装到用户自己的 PATH，例如 `~/.local/bin/herdr-mcp`。该发行 target 支持 `herdr-mcp version` 与只读的 `herdr-mcp update check`。macOS 托管的 `herdr-mcp install` / `service install` 生命周期、自动 `update apply` 与安全 `worker connect` 凭据持久化在 0.4.8 仍不是 Linux 产品面；实际运行 Linux runtime 时，对前台 `candidate` / `link run` 进程使用外部 supervisor。
