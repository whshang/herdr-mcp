...
5. 创建或打开 ChatGPT **Project**。ChatGPT 的 App 选择按消息生效：每条由用户手动发送且需要读取新的工作站数据或执行 Herdr 动作的消息，都要重新选择或 mention 你为这个 Connector 设置的 App；默认/示例名称是 `herdr`，也支持自定义名称。扩展会从 ChatGPT 真实 App pill 的 provider-owned `data-keyword` 读取身份，并写入当前 conversation/Project 的既有 binding。后续 Herdr 自动继续、Agent 结果、恢复、handoff 和 Browser Actuation 会通过 `required_apps` 显式重新选择这个已观察到的真实身份；这保证 Herdr 生成的消息不会猜 App 名称，但不会让 App 永久挂在之后的人类消息上。手动后续消息需要 Herdr 时，再次选择或 mention 该 App。
