...
语义 Auto 不在浏览器扩展里配置 Provider 凭证或 endpoint。扩展只调用本机 Herdr Runtime；Runtime 的 Provider 配置只有两层：单机使用 mode-`0600` 的 `config.toml`，全局使用已认证 Cloudflare Worker 的共享 route pool，同一 typed/chat 能力有本机 route 时优先本机。任意命名的 `[semantic.route.<name>]` 都必须显式配置 `transport`、`base_url`、`model` 和 `api_key`，shell/process environment 不承担 Provider 配置。普通 Auto 固定按 typed evaluation route（Jev）-> chat route（LLM）-> 有界脚本兜底；Goal 模式可把 Jev 作为现有 LLM Goal Supervisor 的 advisory semantic prior。Jev 与 LLM route 共用轮换、总 deadline、冷却和有界错误转移。没有语义 Provider 时脚本兜底仍提供基础 Auto；Work Memory/TODO evidence 与确定性安全门继续保持权威。
