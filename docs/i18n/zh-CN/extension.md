...
| 主线 | 问题 | 方向 | 状态 | 首批站点 |
|---|---|---|---|---|
| **A. 网页连续工作** | 网页派活后会停住；回复可能超时或只显示半截；长对话最终需要换会话 | Herdr 状态观察 + 网页会话绑定 + 手动继续 + 自动化 gate + 安全接力 | **已可用**（当前 0.1.55 系列） | 绑定/观察：4 站；ChatGPT Project 共享自动化；普通 ChatGPT / z.ai / DeepSeek 会话级自动化；手动接力：ChatGPT Project + z.ai `/c/<chat_id>` |
