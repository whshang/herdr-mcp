...
macOS 安装或更新 STANDALONE 后还应执行一次 `herdr-mcp doctor`。`standalone status` 说明受管目录里的文件状态；`doctor` 会额外只读取 Google Chrome profile preferences 中 **Herdr 这个固定 extension ID** 的精确条目，并把 Chrome 当前 Load unpacked 的实际路径与上面的受管路径比较。它同时接受受管 `~/.config/herdr-mcp/extensions/standalone/current` 目录和上次安装记录的 Chrome 可见路径。该记录读取自 `~/.config` state 并按稳定字符串比较，因此改变 `--path`、或 macOS 拒绝读取 `~/Documents`，都不会误报 `drift`。若出现 `WARN standalone-extension-load state=drift`，说明相同固定 ID 仍从另一目录加载，常见于旧的 Downloads/开发副本仍留在 Chrome。打开 `chrome://extensions`，找到 Herdr 扩展，按 `doctor` 给出的 `expected` 路径重新 Load unpacked/Reload。不要仅为修复路径漂移而切换 Native Host channel、删除扩展数据或复制凭据。该检查只诊断，不会重写 Chrome 配置，也不会刷新标签页。
...
设置页只把常用项放在首屏：语言、ChatGPT Project Auto 能力开关和已支持 WebChat 的站点访问。实验性站点、Page Assist、本机连接、连续工作模板和时间参数默认折叠，需要时再展开。workspace binding 与 WebChat Control 继续留在 Side Panel / HUD，因为那里能同时看到当前页面上下文。

语义 Auto 不在浏览器扩展里配置 Provider 凭证或 endpoint。扩展只调用本机 Herdr Runtime；Runtime 的 Provider 配置只有两层：单机使用 mode-`0600` 的 `config.json`，全局使用已认证 Cloudflare Worker 的共享 route pool，同一 typed/chat 能力有本机 route 时优先本机。两层都使用统一的 `name / protocol / url / model / api_key` route JSON 对象；`decision` 与 `decision-vercel` 自动归为 typed evaluation，`openai-chat` 自动归为 chat；shell/process environment 不承担 Provider 配置。普通 Auto 固定按 typed evaluation route（Jev）-> chat route（LLM）-> 有界脚本兜底；Goal 模式可把 Jev 作为现有 LLM Goal Supervisor 的 advisory semantic prior。route 的配置顺序就是优先级：第一个健康 route 为 primary，后续 route 只在失败或 cooldown 后有界接替；Jev 与 LLM route 的单次 timeout 与整个 pool 的总 deadline 分开。旧 `config.toml` 首次读取时会迁移为 JSON。没有语义 Provider 时脚本兜底仍提供基础 Auto；Work Memory/TODO evidence 与确定性安全门继续保持权威。

1.0 的浏览器支持覆盖 ChatGPT、Claude 与 Grok。三者在 Project / Project 对话中统一提供 workspace binding 与 WebChat Control；回到全局新聊天 / 首页后保留 WebChat Control，但没有 Project 或 conversation 作用域时不显示 workspace binding。ChatGPT 另外支持会话 `create`、archive 与 self-handoff；Claude 与 Grok 支持已登录会话的 dispatch、settled result 与 reload recovery。Gemini 保持可选实验状态，不进入 1.0 验收边界。
