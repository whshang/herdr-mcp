...
以 `workspace_id` 为身份事实，label 只是展示缓存。当前扩展会用实时 `/push/events` / `/push/state` workspace catalog 覆盖历史 binding 里的陈旧 label，并自动修复持久化记录。若浮层已显示正确的 `herdr-mcp (w68)` 而底栏仍显示旧项目名，先确认已经加载当前 0.1.46 扩展并刷新页面；不要通过解绑/重绑来“修”同一个 workspace id。
...
0.1.46 的 **手动接力**只对 ChatGPT Project 提供，当前 conversation 必须先绑定 Herdr workspace；普通 `/c/<id>`、Claude、DeepSeek、z.ai 不显示该按钮。绑定 workspace 仍有 agent `working` 时，点击会被后台拒绝并提示等待静止边界，因为接力不能和 settled/wake 的投递目标迁移竞争。
