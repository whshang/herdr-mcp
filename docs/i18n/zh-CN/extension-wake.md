...
HUD 只保留 `手动继续 / herdr监控 / LLM 分析` 三个人工推进动作，当前作用域 `自动 开` 时会锁定，避免和自动状态机同时推进同一会话。**手动接力只有一个 UI 入口：Control Center → 当前页面**；它是例外，支持的会话在 `自动 开/关` 两种状态都可启动，transfer 期间源会话自动 wake 暂停，target 继承 source 的 Auto 状态。
