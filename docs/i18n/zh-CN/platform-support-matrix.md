...
| --- | --- | --- | --- | --- | --- |
| macOS（Apple Silicon） | **生产支持** | 远程文件/Git mutation 仅允许作用于实时 managed Git root，并叠加 read-only/write-root、dirty/busy、secret-path 等 gate。Shell 仍以登录用户身份执行，**不是 sandbox**。涉及 TCC 的原生访问使用长期稳定的 Herdr-MCP broker responsibility boundary。 | 用户级 launchd 管理 runtime 与 production Link；immutable generation 通过 `runtime/current` 切换。本地浏览器 IPC 使用 Herdr-MCP 所有、mode-0600 的 Unix socket；device/runtime credential 使用 macOS 凭据边界。 | Runtime MCP 仅绑定 loopback 并要求本地 bearer；workstation 主动建立经过认证的 outbound Link，不公开 workstation 入站端口。 | **已做实体机验证。** Apple Silicon macOS 是当前主要生产路径；TCC broker/process、generation rollback、OAuth/Connector、真实 ChatGPT 与浏览器插件链路均有专项回归/UAT。 |
| Linux x86_64（原生 Debian 类主机） | **生产支持** | 与 macOS 相同的 managed-Git-root 和 mutation gate，通过 Unix 文件所有权/权限执行。Linux 没有 macOS TCC；shell 同样是 workstation 用户的非 sandbox shell。 | 优先 `systemd --user`；没有 user systemd manager 时使用已验证的 current-user process backend。Device credential 与 Herdr-MCP 所有的 service/control 文件使用私有用户级存储与权限。 | 同样使用 loopback runtime + authenticated outbound Link。 | **已做实体机验证。** 原生 Debian existing-fleet 安装、Link/service lifecycle、updater apply 已验证；发布静态 `x86_64-unknown-linux-musl` 资产。 |
| Linux ARM64 / aarch64（原生 Debian 类主机） | **生产支持** | 与 Linux x86_64 相同的 managed-root 与 Unix ownership 边界。 | 同样使用 `systemd --user` / managed-process service model 和私有 credential store。 | 同样使用 loopback runtime + authenticated outbound Link。 | **已在 NanoPi R5C / Debian 11 ARM64 做实体机验证。** exact-source release build、immutable-generation install、原 enrolled device identity 保留、systemd-user service/Link reconcile 与 Edge `herdr_inspect` 均通过；Release qualification 还在 GitHub 原生 ARM64 runner 构建并 smoke 静态 `aarch64-unknown-linux-musl`。 |
| Windows x86_64 | **候选——非生产支持** | 实体机 UAT 已实际覆盖原生 managed-root/path 边界，但 1.0 Production 声明仍需在最新 main 的精确 candidate 上完成 [#394](https://github.com/whshang/herdr-mcp/issues/394) 的完整晋级记录。 | [#364](https://github.com/whshang/herdr-mcp/pull/364) 的原生实现已进入 `main`，使用 current-user process、Windows Credential Manager、Startup-folder 登录启动项，并做 process ownership fencing。1.0 runtime 也已经在 Windows 上启用首台设备 `worker bootstrap`，包含 canonical enrollment 和受管 Link 激活。源码合并和 hosted CI 只是候选验证证据，不构成正式支持保证。实体机 UAT 已实际跑过安装、service/Link 恢复、模拟登录恢复、device/Connector 保持以及真实 Connector 对话；[#514](https://github.com/whshang/herdr-mcp/pull/514) 修复了验收过程中暴露的 Windows HOME/PATH 问题。 | 候选实现沿用 loopback + authenticated outbound Link。 | **Windows x86_64 已实际进行实体机 UAT，但 #394 的 Production 晋级仍保持打开。** 在冻结最终 1.0 支持声明前，需要在精确 candidate 上记录一次全新 Windows 首台设备 `worker bootstrap`，覆盖 Worker 创建、canonical enrollment、Link ready 和一次真实 MCP 调用；其余晋级记录还需包含 device/Connector 身份保持、模拟登录恢复、最终只读 inspect/workspace/pane 访问和 managed-root 路径证据。没有显式 fixture 前，UNC/网络路径仍不属于支持声明；若要声明 UNC/网络路径能力，必须先补对应 fixture。 |
| Windows ARM64 / aarch64 | **候选——非生产支持** | 与 Windows x86_64 相同的候选 filesystem/path contract；尚不声明实体机文件系统能力。 | 同样使用原生 Windows current-user process、Credential Manager 与 Startup-folder ownership model。 | 同样使用候选 loopback + authenticated outbound Link。 | **已在 GitHub 原生 `windows-11-arm` runner 做 release build/qualification，尚无 Windows ARM64 实体机 UAT。** 发布 target 为 `aarch64-pc-windows-msvc`；晋级 Production 仍需实体机 lifecycle、credential、path 与端到端 Link UAT。 |
