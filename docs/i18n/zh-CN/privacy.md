...
2. **受支持与实验性的 Web AI 网站。** 扩展只在产品文档声明的浏览器页面上运行，用于观察当前对话状态并执行用户可见的连续工作/恢复交互。ChatGPT 是主要支持面，Claude 使用文档所述适配器；z.ai 与 DeepSeek 属于实验性集成，默认关闭，只有用户在 Herdr 设置中显式开启对应开关后才会注册其 content script。
