...
交互式 `update` / `update apply` 会继续把最终机器可读结果写到 stdout，同时把人类可读进度写到 stderr：检查 Release/provenance、验证 artifact attestation、按百分比显示下载进度、校验 candidate、显示 installer 状态，并最终给出经过 health gate 的成功/失败结果。真正的 activation 仍由 detached update worker 执行；前台 CLI 只在有界时间内观察 durable update job，不自行执行 service lifecycle mutation。同 contract 的 Runtime 更新成功提交后，交互式命令还会检查用户自己配置的 Cloudflare Worker；若其 `edgeVersion` 落后于已验证 Release，Herdr 会临时取得 Cloudflare 授权，只对 health 已证明的同一个 Worker 做原地更新，保留 Worker identity、Durable Objects、secrets、public OAuth origin、已登记设备、Connectors 和已知的可选 artifact R2 binding，并且绝不修改 DNS。如果 Cloudflare 授权或 Worker reconcile 无法完成，Runtime 仍保持已更新状态，结果会明确给出 `next_action: herdr-mcp worker update`。当 Runtime 已经是目标版本时再次运行 `herdr-mcp update`，只会补做待完成的 Worker reconcile，不会重装 Runtime。如果安装超过前台观察窗口，命令会返回 `update_queued` 并带 `next_action: herdr-mcp update status`；新 Runtime 生效后，`update status` 会报告 Worker drift。`update auto` 继续保持非交互、静默后台模式，并且绝不会弹出 Cloudflare 授权；Edge 落后时只报告下一步 `herdr-mcp worker update`。已经发布且不可变的 v0.4.2 binary 早于这套进度 UI，因此从 v0.4.2 执行一次 `update apply` 时，网络阶段仍可能暂时没有输出。

`herdr-mcp worker update` 是已有 fleet 的直接 Edge reconcile 命令。正常用户路径要求默认实例已经 enrollment 且当前 Runtime 为 PROD Release，并使用临时 Cloudflare 授权。fleet 管理 preflight 可以识别精确冻结 hash 的 epoch-2 rollback contract，从而让已经 enrollment 的旧 Worker 原地升级；但 Link/install 数据面的 admission window 仍保持 current/N-1，在 Worker 升级完成前继续 fail-closed。对于精确的 DEV/UAT candidate，只有显式设置 `HERDR_MCP_EDGE_BUNDLE_PATH` 指向同一个已资格验证 artifact 内的 Edge bundle 时才允许执行。更新仍拒绝 Edge downgrade 或未知 contract hash；遇到未知/自定义 Worker binding 时 fail-closed，不会静默丢配置。它不会创建第二个 Worker、轮换设备/Connector 凭据、改变 OAuth/DNS identity，也不要求删除并重新添加 ChatGPT Connector。

`update apply` 对不兼容的持久化 state schema 继续 fail-closed。普通 v0.4.8（schema 5）用户升级到 v1.0（schema 15）时，**只需要执行一次 `herdr-mcp update`**。不可变的 v0.4.8 updater 会先接纳 schema-5 的 v0.4.9 migration bridge；该 detached bridge 验证最终 schema-15 Runtime 的 attestation，再调用最终 Runtime 自己的 `update major-apply`，最后原地 reconcile 已经证明身份的 Worker。成功路径不会把 bridge 安装成 production service。

`update major-apply` 继续作为这条用户路径背后的权威维护者/恢复 primitive。它会先停止正在运行的旧服务，使最终快照包含停机期间已提交的写入，再生成私有、一致的 schema-5 SQLite 备份和一份可执行的旧 runtime binary 私有副本，对两份 rollback artifact 都做哈希校验，最后复用正常的事务式 service install 完成迁移和激活。备份失败时会尝试重新启动旧服务，并报告重启失败；若检测到已存在的 rollback 材料，会在停止服务前直接拒绝，避免重试覆盖此前的备份。`update major-rollback` 会先校验回滚记录和旧 binary，再停止当前服务，随后校验数据库备份哈希并恢复升级前的数据库并重新安装备份的旧 runtime；私有 binary 副本使回滚不依赖后续 runtime generation GC。回滚接受中断在 5 至 15 之间任意 schema 的迁移，并会清除 v1.0 迁移后新产生的持久化状态。这两个 major lifecycle 命令都会拒绝在 managed `herdr_exec` 会话内执行。
...
`doctor` 回答的是**“这套安装现在健康吗？”**。默认输出只保留给人看的摘要：本机健康、Edge 可达性、未执行的远程 Connector 认证，以及真正需要处理的项目。完整分层诊断使用 `herdr-mcp doctor --verbose`，机器读取使用 `herdr-mcp doctor --json`。macOS 的 JSON `standalone_extension` 对象会报告受管 STANDALONE 扩展状态；`state=drift` 表示 Google Chrome 正在从 `~/.config/herdr-mcp/extensions/standalone/current` 之外的路径加载同一个固定 standalone extension ID，对象会给出 `expected_path`、实际加载路径/profile、Chromium `location` 和 `drift_count`。浏览器扩展属于可选能力，因此这条提示不会把原本健康的核心 MCP/service readiness 判成失败。`scan` 回答的是**“这台机器上的 Agent 到底有哪些已经被证据确认的能力？”**。
...
传入 `pr_number` 后会返回 PR state、merge state、Auto-merge request、required checks，以及 Deno Deploy 等 supplemental status。每次结果都有确定性的 `fingerprint`；继续监控时把它作为 `previous_fingerprint` 传回，如果状态没有变化，下一次只返回精简 summary 和 `changed=false`，不会重新输出整张检查表。监控正在运行的 CI 时，同时传入 `wait_ms: 20000`：工作站在本机完成这段有界等待，再在同一次调用里执行一次新的 GitHub 状态探测。状态变化时返回完整新状态；没有变化时返回精简的 `changed=false` 并标记 `wait_timeout=true`。20 秒上限为 Edge 的正常请求 deadline 和 GitHub 探测保留余量，也省掉 planner 单独 sleep 再查状态的一次往返。
