...
`update apply` 对不兼容的持久化 state schema 继续 fail-closed。v0.4.8（schema 5）→ v1.0（schema 15）升级，应从独立终端使用已验证的 v1.0 binary 执行 `update major-apply`。该命令会先停止正在运行的旧服务，使最终快照包含停机期间已提交的写入，再生成私有、一致的 schema-5 SQLite 备份和一份可执行的旧 runtime binary 私有副本，对两份 rollback artifact 都做哈希校验，最后复用正常的事务式 service install 完成迁移和激活。备份失败时会尝试重新启动旧服务，并报告重启失败；若检测到已存在的 rollback 材料，会在停止服务前直接拒绝，避免重试覆盖此前的备份。`update major-rollback` 会先校验回滚记录和旧 binary，再停止当前服务，随后校验数据库备份哈希并恢复升级前的数据库并重新安装备份的旧 runtime；私有 binary 副本使回滚不依赖后续 runtime generation GC。回滚接受中断在 5 至 15 之间任意 schema 的迁移，并会清除 v1.0 迁移后新产生的持久化状态。这两个命令都会拒绝在 managed `herdr_exec` 会话内执行。
