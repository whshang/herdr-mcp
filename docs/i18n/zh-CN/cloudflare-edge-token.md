...
它不会为了方便默认申请全账号管理员权限，也不会默认申请 R2 权限。私有 R2 artifact relay 是显式可选能力：只有用户明确启用并接受 Cloudflare 的 R2 订阅/支付步骤后，才使用单独具备 **Workers R2 Storage Write** 能力的部署凭据进行 bucket provisioning。
