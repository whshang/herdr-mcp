...
deterministic な task fact が durable になった後、Parent の優先観測境界は `herdr_mcp.agent.task.inbox(advisory=true)` です。未 acknowledge の terminal work がある場合だけ、Runtime は scope 内の child summary を最大 16 件 freeze し、その batch に対して既存の `agent.attention.advise` を一度だけ評価します。active sibling も同じ request に含められるため、fan-out で child ごとの Jev call は発生しません。inbox fast path の semantic budget は 1.5 秒です。`verify_completion` は deterministic な change projection / validation へ進み、`continue_unobserved` は独立して running 中の child をそのままにし、human/blocker/drift state は次の観測だけを指示します。semantic が未設定、遅延、malformed、error の場合も durable task fact と terminal wake は変わらず、Browser 側に二重の semantic wait や probability threshold はありません。validation、closeout、cleanup は後段の別 boundary で、validation advice は freeze 済み check の順序だけを変え、closeout は optional な post-validation classification、cleanup advice は `safe_to_delete` や reclaim authority を変更できません。Herdr 0.9.x には native prompt-turn identity がないため、安定した Runtime `turn_id` は herdr-mcp の turn envelope であり、`exact_native_turn=false` を明示します。
