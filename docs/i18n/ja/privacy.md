...
- **認証情報:** ユーザーがその機能を明示的に設定した場合の、ユーザー設定の OpenAI 互換 LLM endpoint 用の任意の API key。
...
- 任意でユーザーが設定する LLM endpoint、モデル、API key。
...
3. **ユーザー設定の LLM endpoint。設定済みの LLM 機能に限ります。** ユーザーが OpenAI 互換 LLM endpoint を設定した場合、その設定を保存またはテストする時点で、Chrome はその endpoint の正確な origin へのアクセスを求めます。その後拡張は、任意のターン後分析のために、関連する user/assistant テキストとユーザー提供の API 資格情報をその endpoint へ送信できます。会話の handoff がユーザーによって起動された場合、またはユーザーが有効にした Auto ポリシーによってトリガーされた場合で、現在の Web AI 会話が、会話のハードリミットに達した、handoff プロンプトを送信できない、または主たる要約が有効な packet なしに settle した、という理由で必要な handoff サマリを生成できないとき、拡張は代わりに有界なソース transcript を同じ設定済み endpoint へ送信して handoff packet を生成できます。フォールバック transcript には、拡張が選択した user/assistant の会話テキストのみが含まれ、拡張の handoff 上限（現在 70,000 文字。切り詰めが必要な場合は初期のタスクフレーミングと直近の操作状態を保持）に制限されます。endpoint はユーザーが選択するものであり、既定では Herdr の publisher が選択または運用するものではありません。endpoint プロバイダー自身のプライバシーと保持の条件が適用されます。
...
- host access — 常時有効なアクセスは、文書化された ChatGPT/Claude の面とローカルの herdr-mcp endpoint に限定されます。実験的な z.ai/DeepSeek へのアクセスとユーザー設定の LLM endpoint は、ユーザーが明示的に有効化または設定した後にのみ要求される Chrome の任意の host permission を使用します。Herdr は `<all_urls>` を常時有効な host permission として要求しません。
...
拡張が開始する公衆ネットワーク接続は、該当する場合に HTTPS/WSS を使用します。拡張と同一コンピュータ上の native program との間の Native Messaging 通信はローカルに留まります。任意の LLM API key などのシークレットは、プロジェクトリポジトリや publisher の telemetry に意図的に書き込まれることはありません。
