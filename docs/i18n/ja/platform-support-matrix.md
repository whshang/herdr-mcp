...
| Windows x86_64 | **Candidate — not production** | Windows レーンが実機で managed-root/path の挙動（Windows ネイティブのパスエッジケースを含む）を証明するまで、1.0 のプロダクション主張はありません。 | [#364](https://github.com/whshang/herdr-mcp/pull/364) のネイティブ実装は `main` にあり、current-user プロセス、Windows Credential Manager、Startup フォルダのログインエントリを使用し、process ownership fencing を備えています。ソースのマージと hosted CI は qualification の証拠であり、プロダクションの保証ではありません。物理的なプロモーションは [#394](https://github.com/whshang/herdr-mcp/issues/394) で追跡されています。 | Candidate は同じ loopback + 認証済み外向き Link モデルに従います。 | **hosted Windows CI/release build はテスト済みで、物理 UAT はまだありません。** #394 に基づくプロモーションの前に必須: identity を失わない再インストール、Herdr の復旧、シミュレートログインからの復旧、Connector/device の保持、そして実際の ChatGPT → Edge → Link → Windows → Herdr 読み取り経路の最終確認。より広いファイルシステム主張の前に、UNC/パスの挙動をカバーする必要があります。 |
