...
| Workstation Runtime Execution Contract | epoch **3**, **18** tools | The immediately previous frozen epoch **2**, **18** tools is accepted as the bounded rollback baseline | Any other epoch/hash pair is rejected before workstation execution |
...
- **N-1 runtime execution** is a bounded rollback path, not a general compatibility promise. Today that means the immediately previous frozen epoch-2 runtime contract remains accepted beside current epoch 3. The pair is validated by epoch and hash; arbitrary older catalogs are not accepted.
