...
| Windows x86_64 | **Candidate — not production** | Physical-machine UAT has exercised the native managed-root/path boundary, but the 1.0 Production claim still requires the full [#394](https://github.com/whshang/herdr-mcp/issues/394) promotion record against the exact current-main candidate. | The native implementation from [#364](https://github.com/whshang/herdr-mcp/pull/364) is on `main` and uses current-user processes, Windows Credential Manager and a Startup-folder login entry, with process ownership fencing. Source merge and hosted CI are qualification evidence, not a production guarantee. Physical UAT exercised install/service/Link recovery, simulated-login recovery, preserved device/Connector state and a real Connector conversation; [#514](https://github.com/whshang/herdr-mcp/pull/514) fixed the Windows HOME/PATH defects exposed during qualification. | Candidate follows the same loopback + authenticated outbound Link model. | **Physical x86_64 UAT has been exercised, but Production promotion remains open under #394.** The final promotion record must be rerun/recorded on the exact current-main candidate and include preserved device/Connector identity, simulated-login recovery, final read-only inspect/workspace/pane access, and managed-root path evidence. UNC/network paths remain outside the support claim without an explicit fixture; UNC/path behavior must be covered before any broader filesystem claim. |
