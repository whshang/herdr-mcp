...
| Public Edge tool contract | epoch **7**, **19** actions | A client conversation may retain an older tool snapshot, but Edge publishes the current DEV/PROD public catalog; the epoch-3 identity remains only as the conservative non-DEV/PROD fallback and rollback baseline | Tools outside the current public contract are rejected |
| Workstation Runtime Execution Contract | epoch **4**, **18** tools | The immediately previous frozen epoch **3**, **18** tools is accepted as the bounded rollback baseline, together with the frozen epoch **2**, **18** tool catalog | Any other epoch/hash pair is rejected before workstation execution |
...
- **N-1 runtime execution** is a bounded rollback path, not a general compatibility promise. Today that means the immediately previous frozen epoch-3 runtime contract, plus the frozen epoch-2 catalog still present in the field, remain accepted beside current epoch 4. The pair is validated by epoch and hash; arbitrary older catalogs are not accepted.
...
| --- | --- | --- | --- | --- | --- |
| macOS (Apple Silicon) | **Production** | Remote file/Git mutation is restricted to live managed Git roots, with read-only/write-root, dirty/busy and secret-path gates. Shell execution remains the logged-in user's shell and is **not** a sandbox. TCC-sensitive native access uses the long-lived Herdr-MCP broker responsibility boundary. | User launchd owns the managed runtime and production Link; immutable generations switch through `runtime/current`. Local browser IPC uses an owned mode-0600 Unix socket; device/runtime credentials use the macOS credential boundary. | Runtime MCP stays on loopback behind a local bearer. The workstation opens the authenticated outbound Link to Edge; no inbound workstation listener is exposed publicly. | **Physically qualified.** Apple Silicon macOS is the primary production path; TCC broker/process, generation rollback, OAuth/Connector, real ChatGPT and browser-extension paths have dedicated regression/UAT coverage. |
| Linux x86_64 (native Debian-class host) | **Production** | Same managed-Git-root and mutation gates as macOS, enforced with normal Unix filesystem ownership/permissions. There is no macOS TCC equivalent; shell remains the workstation user's unsandboxed shell. | `systemd --user` is preferred; the managed current-user process backend is the qualified fallback when a user systemd manager is unavailable. Device credentials and owned service/control files use private per-user storage/permissions. | Same loopback runtime + authenticated outbound Link model. | **Physically qualified.** Native Debian existing-fleet installation, Link/service lifecycle and updater application have been qualified; the release artifact is static `x86_64-unknown-linux-musl`. |
| Linux ARM64 / aarch64 (native Debian-class host) | **Production** | Same Linux managed-root and Unix ownership boundary as x86_64. | Same `systemd --user` / managed-process service model and private credential store. | Same loopback runtime + authenticated outbound Link model. | **Physically qualified on NanoPi R5C / Debian 11 ARM64.** Exact-source release build, immutable-generation install, preserved enrolled device identity, systemd-user service/Link reconciliation and Edge `herdr_inspect` all passed. Release qualification also builds and smokes static `aarch64-unknown-linux-musl` on a native GitHub ARM64 runner. |
| Windows x86_64 | **Candidate — not production** | Physical-machine UAT has exercised the native managed-root/path boundary, but the 1.0 Production claim still requires the full [#394](https://github.com/whshang/herdr-mcp/issues/394) promotion record against the exact current-main candidate. | The native implementation from [#364](https://github.com/whshang/herdr-mcp/pull/364) is on `main` and uses current-user processes, Windows Credential Manager and a Startup-folder login entry, with process ownership fencing. The 1.0 runtime also enables first-device `worker bootstrap` on Windows, including canonical enrollment and managed Link activation. Source merge and hosted CI are qualification evidence, not a production guarantee. Physical UAT exercised install/service/Link recovery, simulated-login recovery, preserved device/Connector state and a real Connector conversation; [#514](https://github.com/whshang/herdr-mcp/pull/514) fixed the Windows HOME/PATH defects exposed during qualification. | Candidate follows the same loopback + authenticated outbound Link model. | **Physical x86_64 UAT has been exercised, but Production promotion remains open under #394.** Before the final 1.0 claim is frozen, record one fresh Windows first-device `worker bootstrap` on the exact candidate through Worker creation, canonical enrollment, Link readiness, and a real MCP call. The remaining promotion record must also include preserved device/Connector identity, simulated-login recovery, final read-only inspect/workspace/pane access, and managed-root path evidence. UNC/network paths remain outside the support claim without an explicit fixture; UNC/path behavior must be covered before any broader filesystem claim. |
| Windows ARM64 / aarch64 | **Candidate — not production** | Same candidate filesystem/path contract as Windows x86_64; no physical-machine filesystem claim yet. | Same native Windows current-user process, Credential Manager and Startup-folder ownership model. | Same candidate loopback + authenticated outbound Link model. | **Native GitHub `windows-11-arm` release build/qualification tested; no physical Windows ARM64 UAT yet.** The published target is `aarch64-pc-windows-msvc`; production promotion remains gated on physical-machine lifecycle, credential, path and end-to-end Link UAT. |
