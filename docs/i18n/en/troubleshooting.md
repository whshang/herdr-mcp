...
Version 0.1.46 exposes **Manual handoff** only for ChatGPT Project conversations, and the current conversation must first be bound to a Herdr workspace. Plain `/c/<id>`, Claude, DeepSeek, and z.ai do not show the button. If a bound workspace still has an agent `working`, the background rejects the handoff and asks you to wait for a quiescent boundary because binding cutover must not race settled/wake delivery.
