...
If z.ai is already on `/c/<chat_id>` but the HUD still looks like the root launcher, verify that 0.1.52 is loaded and refresh once. A temporary binding/automation preference created on z.ai `/` migrates once when the same tab first becomes `/c/<chat_id>`; later navigation between existing `/c/A` and `/c/B` chats never drags it along. z.ai handoff summary/seed messages use the raw send path and are not rewritten by the JSON→MCP bridge.
