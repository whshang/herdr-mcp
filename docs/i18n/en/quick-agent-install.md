...
1. Download the newest stable `herdr-mcp` binary for this platform from <https://github.com/whshang/herdr-mcp/releases> (`v0.4.1` is the current published stable tag at this snapshot; `v0.4.2` is release-ready source but is not published until the signed tag gate passes; always prefer the newest published stable tag).
