...
4. Bind the web chat to the working workspace in the MV3 extension. Options chooses **Manual globally / Per-Project automation**. In Per-Project mode, explicitly enable the current ChatGPT Project from its HUD before automatic progress/settled wakes, LLM continuation, timeout recovery or safe rollover can run; otherwise **Manual continue / Herdr monitor / LLM analysis** take over. When you want to roll over early, a bound Project can use **Manual handoff** even with `Auto on`. All extension traffic stays localhost-only; the static token never crosses Cloudflare. See [extension-wake](extension-wake.md).
