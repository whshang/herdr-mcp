...
For an **existing fleet**, do not rerun first-Worker bootstrap. Use `herdr-mcp worker update`, or use the normal interactive `herdr-mcp update` and let it reconcile Edge after the Runtime update succeeds. Existing-fleet update downloads the exact release-matched Edge bundle and verifies its digest and GitHub attestation before temporary Cloudflare authorization. It reads the current Worker health identity and Cloudflare settings before mutation, updates only that proven script, keeps the existing Durable Objects and secrets, carries forward the known optional `ARTIFACT_BUCKET` binding, preserves the configured public OAuth origin (including a Custom Domain), and leaves routes/DNS, devices and Connectors untouched. Unknown custom bindings fail closed rather than being discarded. The exact frozen epoch-2 rollback identity is recognized only by the fleet-management preflight so an already-enrolled legacy Worker can be reconciled in place; arbitrary/unknown Runtime execution contracts remain an explicit migration boundary, and Link/install admission is not widened. `update auto` never opens Cloudflare authorization; it reports `herdr-mcp worker update` when Edge reconciliation is still required.
