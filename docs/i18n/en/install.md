...
On an x86_64 Debian server, use the `herdr-mcp-<version>-x86_64-unknown-linux-musl` asset. Verify its checksum/attestation through the release manifest, then install the raw binary into a user-owned PATH location, for example `~/.local/bin/herdr-mcp`. `herdr-mcp version` and the read-only `herdr-mcp update check` are supported against this release target. The macOS-managed `herdr-mcp install` / `service install` lifecycle, automatic `update apply`, and secure `worker connect` credential persistence are not Linux product surfaces in 0.4.8; use an external process supervisor for foreground `candidate` / `link run` processes when operating the Linux runtime.
