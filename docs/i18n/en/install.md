...
### What `herdr-mcp-broker` is on macOS

`herdr-mcp-broker` is a **macOS-only** stable TCC / Full Disk Access broker. Linux and Windows neither install nor require it. It was introduced in v0.4.2; v0.4.3 completed its first-install and upgrade lifecycle. Its fixed path is `~/.config/herdr-mcp/tcc-broker/herdr-mcp-broker`.

The broker exists because macOS privacy access to folders such as `Documents`, `Desktop`, and `Downloads` is tied to a relatively stable code identity. `runtime/generations/rust-*` rotates as herdr-mcp is upgraded, so granting Full Disk Access directly to every generation would risk repeated authorization after upgrades. Ordinary runtime updates therefore preserve a broker at the same compatibility revision. The broker is replaced only through an explicit broker-upgrade path when its own compatibility revision genuinely changes.

The broker handles only allowlisted protected-folder file and Git operations behind Herdr's existing gates, such as `fs_read`, `fs_list`, `fs_grep`, `fs_edit`, `fs_write`, `fs_patch`, and `git`. It is **not an arbitrary shell executor**, and granting Full Disk Access to it does not automatically bypass macOS TCC for every Herdr pane, Pi, Codex, or other Agent child process. Those interactive execution paths remain governed by the TCC identity of the Herdr/terminal execution boundary that owns them.

An interactive first install prepares the fixed broker and, when needed, opens **System Settings → Privacy & Security → Full Disk Access**. macOS does not allow herdr-mcp to grant that permission silently; the user must still enable `herdr-mcp-broker`. A non-interactive install prepares the broker without forcing System Settings open. Use these commands for installation and troubleshooting:
...
Only after `verify` reports `granted` should the broker-owned protected-folder MCP file/Git path be considered ready. `sudo` is not a substitute for TCC approval.

When ChatGPT / Web AI assists with installation, it must teach the concrete macOS UI flow rather than only showing a command or error: open **Privacy & Security → Full Disk Access**; enable `herdr-mcp-broker` if already listed, or choose `+`, press `Command+Shift+G` in the file picker, enter `~/.config/herdr-mcp/tcc-broker/herdr-mcp-broker`, and add it. Touch ID / password confirmation must be completed by the user. The Agent then reruns `herdr-mcp permissions verify`; only `status: granted` / `probe: granted` counts as completion.
...
On macOS the service remains a normal user LaunchAgent; `sudo` is not required and does not grant TCC access. For MCP file/Git paths, the permission target is the stable `~/.config/herdr-mcp/tcc-broker/herdr-mcp-broker`, not a rotating `runtime/generations/rust-*` binary. A non-interactive install prepares the broker but does not open System Settings; run `herdr-mcp permissions setup` once at a user terminal if Full Disk Access is not already granted. Arbitrary shell execution in Herdr panes/Agents does not pass through this broker; when such a process directly accesses a macOS protected folder, troubleshoot the TCC boundary of the corresponding Herdr/terminal execution host rather than assuming broker `granted` applies to every child process.
