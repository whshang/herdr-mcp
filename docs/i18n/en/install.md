...
On Debian, use the static musl release asset matching the machine architecture: `x86_64-unknown-linux-musl` for x86_64 or `aarch64-unknown-linux-musl` for ARM64. The installer prefers `systemd --user`; when no user systemd manager exists it uses the managed user-process backend. On macOS, the service is a user LaunchAgent and Full Disk Access is granted only to the stable macOS-only broker at `~/.config/herdr-mcp/tcc-broker/herdr-mcp-broker`; Linux and Windows do not use that TCC/FDA path. `sudo` does not replace macOS privacy permission.
