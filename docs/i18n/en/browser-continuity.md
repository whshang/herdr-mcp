...
Handoff has one durable recovery payload. `herdr_mcp.browser_handoff.prepare` reuses the existing `continuity_id`, keeps the exact old ChatGPT URL as `source_url`, and returns one compact continuation message for both automatic delivery and Copy Prompt fallback. A user without browser automation can open a new Web-AI conversation and paste that same message. The old URL also remains a standalone recovery anchor: when only the URL is available, the planner can call `continuity.resume` with the full URL as `conversation_url`; only `continuity_ambiguous` falls back to bounded `continuity.search`. This recovery does not depend on ego-browser or prior endpoint/account/Project/generation/device enumeration. A private ChatGPT conversation that was never captured into the local journal cannot be fetched from its URL through MCP alone because the MCP connection does not carry the user's private ChatGPT session body.
...
If the local journal is unavailable or live Rust resolve fails, a new ChatGPT handoff fails closed while preserving the source conversation and full source URL. It no longer asks the source WebChat or a fallback LLM to generate a handoff summary. `HERDR_HANDOFF_V1` remains read/recovery compatibility only for already-existing legacy transfers and provider-specific legacy contracts such as z.ai; new ChatGPT handoffs do not create it.
...
The HUD exposes Continue / Check Herdr / LLM decide plus Handoff. The first three page-scoped progression actions are locked while Auto is on; Handoff remains available when its safety gates pass. Workspace binding and local Herdr controls stay in the Side Panel. Handoff has a single UI entry in the **HUD**, pauses source automatic wakes during transfer, and makes the target inherit the source Auto state.
...
When the inbox contains unacknowledged terminal work, Runtime may evaluate one bounded `agent.attention.advise` batch for up to 16 scoped child summaries, including active siblings. The result is attached to the inbox as Parent guidance: `verify_completion` enters deterministic validation, `continue_unobserved` leaves independent running siblings alone, and human/blocker/drift states direct the next observation. Terminal facts still wake the Parent immediately; the Browser no longer waits for a second semantic grace or applies its own probability thresholds. If Jev is unconfigured, slow, malformed, or fails, the same deterministic terminal wake proceeds.
...
The extension does not configure provider endpoints, models, or API keys. It calls the local Herdr Runtime semantic service. Provider configuration has exactly two layers: mode-`0600` `config.json` for the workstation and the authenticated Cloudflare Worker route pool shared across enrolled workstations; local routes take precedence per typed/chat capability. Both layers use the same `name / protocol / url / model / api_key` route JSON object; `decision` and `decision-vercel` imply typed evaluation, while `openai-chat` implies chat; shell/process environment is not a provider configuration source. Configuration order is route priority; later routes are bounded fallbacks after failure/cooldown. Typed and chat routes share bounded per-route attempts, separate total deadlines, cooldown, and serial failover.
...
Handoff is useful at a natural work boundary before the current conversation becomes difficult to manage. It has one product action in the **in-page HUD**; the Side Panel does not duplicate it. The same canonical package also supports WebChat self-handoff and a user copying the prompt into a newly opened conversation.

It is supported for bound ChatGPT Project conversations and stable z.ai `/c/<chat_id>` conversations. Handoff can start with Auto on or off; the target conversation inherits the source Auto state. Automatic wakes from the source pause while the transfer is active. For ChatGPT, cutover changes only the Project binding's active conversation target; for z.ai it migrates the conversation-scoped binding. The workspace must not have active working agents, so settled/wake delivery cannot race the cutover.

For ChatGPT, Rust first resolves the durable Continuity Journal and calls the read-only `herdr_mcp.browser_handoff.prepare`. The returned package contains the existing `continuity_id`, exact `source_url`, optional `work_chain_id`, minimal target context, and one compact message. `automatic_delivery.params.message` and `manual_delivery.copy_prompt` are byte-for-byte the same message. Automatic delivery passes it unchanged to `browser_session.create`; the new conversation begins by calling `continuity.resume` and then re-checks live workspace / Git / runtime state. If browser control is confirmed unavailable, the HUD exposes **Copy Prompt** from that already-prepared package. A `seed_uncertain` / uncertain mutation outcome does not expose Copy Prompt until reconciliation proves the first delivery did not apply, so Herdr cannot create a second conversation by guessing. If automatic delivery returns no Herdr execution or result fields, that outcome provides no evidence that workstation execution occurred; use the already-prepared Copy Prompt or re-observe the exact dispatch when one exists. Herdr-reported uncertain delivery remains reconciliation-only and is never replayed automatically. Existing provider-specific legacy transfers keep their compatibility recovery contract, but new durable ChatGPT handoffs have no second prompt builder.
...
7. an explicit handoff changes the ChatGPT Project active target only after the new seed is verified (or migrates the binding after confirmation on conversation-scoped sites); a confirmed delivery failure exposes the same canonical Copy Prompt, while an uncertain delivery does not.
