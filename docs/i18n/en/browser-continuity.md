...
The extension does not configure provider endpoints, models, or API keys. It calls the local Herdr Runtime semantic service. Provider configuration has exactly two layers: mode-`0600` `config.toml` for the workstation and the authenticated Cloudflare Worker route pool shared across enrolled workstations; local routes take precedence per typed/chat capability. Each arbitrary `[semantic.route.<name>]` explicitly declares `transport`, `base_url`, `model`, and `api_key`; shell/process environment is not a provider configuration source. Typed and chat routes share one bounded rotation, deadline, cooldown, and failover policy.
