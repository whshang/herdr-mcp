...
Manual handoff can also start from the old ChatGPT conversation URL itself. The user can paste that URL into a new Web-AI conversation and ask Herdr to continue it; the planner calls `continuity.resume` directly with the full URL as `conversation_url`, and Rust parses the ChatGPT Project/conversation identity and resumes the unique durable chain in the same call. Only `continuity_ambiguous` falls back to bounded `continuity.search` candidate discovery; newest-or-most-similar selection remains forbidden. This recovery does not depend on the browser extension, ego-browser, a target composer, or prior endpoint/account/Project/generation/device enumeration. The extension remains useful for automatic turn capture, automatic rollover, and generating the same source-URL-plus-continuity prompt for the user. A private ChatGPT conversation that was never captured into the local journal cannot be fetched from its URL through MCP alone because the MCP connection does not carry the user's private ChatGPT session body.
...
For a manual ChatGPT Project handoff, Herdr first resolves the durable Continuity Journal and seeds the new Project conversation from that reference without writing a summary request into the source conversation. The target prompt always retains the full old conversation URL for human inspection. If binding metadata is missing or stale, the runtime resolves the same chain from the current conversation identity. If no durable chain exists, the handoff fails closed and leaves the current conversation unchanged; it no longer invokes a fallback LLM. WebChat self-handoff uses `browser_session.create(source_url, message, idempotency_key)`, letting Rust derive the same account, Project, and current generation from the registered source session instead of pre-enumerating browser resources; if `message` does not already contain the old link, the runtime appends the exact original `source_url` to the first message so it remains available for human inspection. Existing legacy transfers and conversation-scoped providers keep their compatibility read/recovery contracts.
