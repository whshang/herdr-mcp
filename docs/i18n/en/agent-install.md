...
- Never run `herdr-mcp worker pair` on the computer currently being installed as a way to discover whether a fleet exists. `worker pair` is an **owner-side** action and requires proof that this machine is already enrolled in that fleet. A fresh machine with no enrolled device identity / existing Edge origin must fail closed with first-owner-or-connect guidance, not fall through to a missing LaunchAgent/Keychain implementation error.
- If the user cannot provide Cloudflare account/origin information and there is also no existing authorized device or authorized WebChat that can mint a pairing, stop and ask the single fleet-ownership question below. Do not guess "existing fleet" and do not invoke `worker pair`. If they choose **join existing**, require a pairing from an already-authorized source. If they choose **first Worker**, continue to the Cloudflare bootstrap and pause only when Cloudflare authorization is actually required.
...
**Existing-Worker detection before deploy.** With `ACCOUNT_ID`, list `GET /client/v4/accounts/<ACCOUNT_ID>/workers/scripts`. If an existing Herdr Worker is visible there — or the user has an existing authorized Herdr WebChat/Connector that can create a pairing — stop the deploy path and switch to the existing-fleet flow in [Multi-device control](existing-worker-connect.md) (create pairing from the authorized source, then `worker connect` here). `worker pair` is only the fallback on an already-enrolled owner computer; never run it on this fresh machine as a detection probe. Deploy a new Worker only after the explicit first-fleet answer from §0.
