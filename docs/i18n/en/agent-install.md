...
## 0.5 Fleet existence gate — before any Cloudflare mutation
...
- If the user says another Herdr computer already exists but no pairing address is available, first use an explicitly approved Herdr WebChat if one is available: `herdr_mcp.device.pair` creates the short-lived pairing at the Worker control plane without routing through a workstation. If that WebChat returns `fleet_admin_required`, run `herdr-mcp worker pair` on **any already-enrolled computer**. Enrolled devices have no owner/member hierarchy. Show the pairing address, one-time 6-digit code, and exact expiry together. Consume the pairing on the new computer with `worker connect` and reuse the existing public MCP/OAuth origin.
...
- If the user cannot provide Cloudflare account/origin information and there is also no enrolled device or explicitly approved WebChat that can mint a pairing, stop and ask the single fleet-existence question below. Do not guess "existing fleet" and do not invoke `worker pair`. If they choose **join existing**, require a pairing from an already-authorized fleet principal. If they choose **first Worker**, continue to the Cloudflare bootstrap and pause only when Cloudflare authorization is actually required.
- If the user asks to permanently remove an enrolled computer, prefer an explicitly approved Herdr WebChat: list devices with `herdr_devices`, select the immutable `device_id`, then call Edge-local `herdr_mcp.device.revoke` with `confirm=true`. Never revoke by display name. If the WebChat lacks fleet authority, run `herdr-mcp worker revoke <device-id> --confirm` from any enrolled computer.
- If the user explicitly confirms this is the first Herdr computer/fleet, continue with the first-Worker Cloudflare path below.
- If intent is unclear, ask exactly one fleet question: **create the first Herdr Worker, or join an existing Herdr Worker?** Do not infer the answer from missing local credentials.
