...
For interactive `update` / `update apply`, the CLI keeps the final machine-readable result on stdout and writes human progress to stderr: release/provenance discovery, artifact attestation, bounded download percentage, candidate verification, installer state, and the final health-gated outcome. The downloaded candidate still performs activation in the detached update worker; the foreground CLI observes the durable update job for a bounded interval instead of making the lifecycle mutation itself. After a same-contract Runtime update has committed, the interactive command also checks the user's configured Cloudflare Worker. If its `edgeVersion` is behind the verified release, Herdr obtains temporary Cloudflare authorization and updates that exact health-proven Worker in place while preserving its Worker identity, Durable Objects, secrets, public OAuth origin, enrolled devices, Connectors and known optional artifact R2 binding; it never changes DNS. If Cloudflare authorization or Worker reconciliation cannot complete, the Runtime stays updated and the result reports `next_action: herdr-mcp worker update`. Running `herdr-mcp update` again when the Runtime is already current performs only this pending Worker reconciliation instead of reinstalling the Runtime. If installation legitimately outlives the bounded foreground interval, the command returns an `update_queued` result with `next_action: herdr-mcp update status`; `update status` reports Worker drift once the new Runtime is active. `update auto` remains non-interactive and never opens Cloudflare authorization, so it reports `herdr-mcp worker update` as the next action when Edge is stale. The immutable v0.4.2 binary predates this progress UI, so its one-time `update apply` can remain quiet while doing network work.

`herdr-mcp worker update` is the direct existing-fleet Edge reconciliation command. Normal user operation requires an enrolled default-instance PROD Runtime and temporary Cloudflare authorization. Fleet-management preflight may identify the exact frozen epoch-2 rollback contract so an already-enrolled legacy Worker can be updated in place, while Link/install data-plane admission remains current/N-1 and still fails closed until the Worker is upgraded. For an exact DEV/UAT candidate, the command is enabled only when `HERDR_MCP_EDGE_BUNDLE_PATH` explicitly points at the Edge bundle from that same qualified artifact. The update refuses Edge downgrade or unknown contract hashes and fails closed rather than dropping unknown/custom Worker bindings. It does not create a second Worker, rotate device/Connector credentials, change OAuth/DNS identity, or require the ChatGPT Connector to be deleted and re-added.

`update apply` intentionally refuses an incompatible durable-state schema. For an ordinary v0.4.8 (schema 5) user, the v1.0 (schema 15) transition is **one command: `herdr-mcp update`**. The immutable v0.4.8 updater first admits the schema-5 v0.4.9 migration bridge; that detached bridge attests the final schema-15 Runtime, invokes the final Runtime's `update major-apply`, and then reconciles the already-proven Worker in place. The bridge is never installed as the production service on the successful path.

`update major-apply` remains the authoritative operator/recovery primitive behind that user path. It first stops the running source service so the final snapshot includes writes committed during shutdown, then creates a private consistent schema-5 SQLite backup plus a private executable copy of the exact old runtime binary, hashes both rollback artifacts, and reuses the normal transactional service install to migrate and activate. A failed backup attempts to restart the source service and reports any restart failure, and preexisting rollback material is rejected before the stop so a retry cannot overwrite an earlier backup. `update major-rollback` verifies the rollback record and source binary, stops the active service, then verifies the database backup hash and restores that exact pre-upgrade database, and reinstalls the backed-up old runtime. The private binary copy keeps rollback independent of later runtime-generation garbage collection. Rollback accepts a migration interrupted at any schema between 5 and 15, then removes state created after the v1.0 migration. Both major lifecycle commands refuse execution inside managed `herdr_exec` sessions.
...
When `pr_number` is present, the result includes the PR state, merge state, Auto-merge request, required checks, and supplemental statuses such as external deployments. Every response has a deterministic state `fingerprint`. Pass that value back as `previous_fingerprint` while monitoring a PR; if nothing relevant changed, the next call returns only compact summary counts plus `changed=false` rather than replaying the complete status table. For active CI monitoring, add `wait_ms: 20000` together with `previous_fingerprint`: the workstation waits locally and then performs one fresh GitHub probe in the same call. A changed state is returned in full; an unchanged state returns compact `changed=false` with `wait_timeout=true`. The 20-second ceiling keeps the operation inside the normal Edge request deadline while replacing a separate planner sleep plus status round trip or `gh run watch`.
