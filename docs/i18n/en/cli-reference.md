...
`update apply` intentionally refuses an incompatible durable-state schema. The v0.4.8 (schema 5) → v1.0 (schema 15) transition uses the verified v1.0 binary's `update major-apply` command from an independent terminal. It first stops the running source service so the final snapshot includes writes committed during shutdown, then creates a private consistent schema-5 SQLite backup plus a private executable copy of the exact old runtime binary, hashes both rollback artifacts, and reuses the normal transactional service install to migrate and activate. A failed backup attempts to restart the source service and reports any restart failure, and preexisting rollback material is rejected before the stop so a retry cannot overwrite an earlier backup. `update major-rollback` verifies the rollback record and source binary, stops the active service, then verifies the database backup hash and restores that exact pre-upgrade database, and reinstalls the backed-up old runtime. The private binary copy keeps rollback independent of later runtime-generation garbage collection. Rollback accepts a migration interrupted at any schema between 5 and 15, then removes state created after the v1.0 migration. Both commands refuse execution inside managed `herdr_exec` sessions.
