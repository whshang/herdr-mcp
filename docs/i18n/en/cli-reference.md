...
`herdr-mcp worker update` is the direct existing-fleet Edge reconciliation command. It requires an enrolled default-instance PROD Runtime and temporary Cloudflare authorization, refuses Edge downgrade or an incompatible Runtime execution contract, and fails closed rather than dropping unknown/custom Worker bindings. It does not create a second Worker, rotate device/Connector credentials, change OAuth/DNS identity, or require the ChatGPT Connector to be deleted and re-added.
