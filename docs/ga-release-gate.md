...
| G3 | **PARTIAL** | Live alpha.9 已识别顶层 `install` / `rollback` / `uninstall`（[#74](https://github.com/whshang/herdr-mcp/pull/74)）；源码侧 `install`/`update` 现会把 `~/.local/bin/herdr-mcp` 链到 `runtime/current`。正式 seal 仍待下一 live 代际带出后跑一次 `install`/`update apply` || G4 | **PARTIAL** | `service install` + generation/health/rollback 代码存在；正式文档未给出 binary → `herdr-mcp install` 干净机路径 |
