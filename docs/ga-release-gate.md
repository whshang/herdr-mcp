...
| G5 | **FAIL** | **关键 blocker / #1**：production Link 仍走 Node（desired=active=stable-0.3.32）；health `production_ready=false` / rust-candidate。alpha.11 已 live 安装候选 LaunchAgent `dev.herdr-mcp.link-rust-candidate`（`runtime/current`，不碰 Node jobs）；生产 LaunchAgent / live cutover / dual UAT 仍未做 |
...
1. **G5 — Rust production Link 切流 + `production_ready`**（#1 critical）：候选 LaunchAgent 已在 alpha.11 本机 soak（`dev.herdr-mcp.link-rust-candidate`）；仍须生产所有权切换、去掉用户路径对 Node link 的依赖、health 不再停在 rust-candidate / `production_ready=false`。切流清单与门闩见 [`docs/_wip/g5-link-production-cutover.md`](./_wip/g5-link-production-cutover.md)；下一步是 production cutover helper + 独立 Shell 双人验收（仍禁止切 Node `link`/`link-prod` 直到明确 cut）。
